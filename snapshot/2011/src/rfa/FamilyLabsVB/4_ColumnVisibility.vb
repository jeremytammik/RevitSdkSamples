...
<Regeneration(RegenerationOption.Automatic)> _
Public Class RvtCmd_FamilyCreateColumnVisibility
    ...
    Sub addLineObjects()
        ...
        Dim pGeomPlaneH As Plane = _rvtApp.Create.NewPlane(normal, pts(1))
        Dim pSketchPlaneH As SketchPlane = _rvtDoc.FamilyCreate.NewSketchPlane(pGeomPlaneH)

        ''  (4) create line objects: two symbolic curves on a plan and one model curve representing a column like a vertical stick.
        ''
        Dim geomLine1 As Line = _rvtApp.Create.NewLine(pts(0), pts(1), True)
        Dim geomLine2 As Line = _rvtApp.Create.NewLine(pts(1), pts(2), True)
        Dim geomLineH As Line = _rvtApp.Create.NewLine(pts(1), ptH, True)
...
#Region "Helper Functions"
    ...
    Function findFace(ByVal pBox As Extrusion, ByVal normal As XYZ, ByVal refPlane As ReferencePlane) As PlanarFace
        ...
        Dim op As Options = _rvtApp.Create.NewGeometryOptions()
    ...
    Function findElement(ByVal targetType As Type, ByVal targetName As String) As Element
        ...
        ''
        Dim collector = New FilteredElementCollector(_rvtDoc)
