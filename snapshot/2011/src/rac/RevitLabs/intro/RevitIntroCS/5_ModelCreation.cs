  ...
  public class ModelCreation : IExternalCommand
  {
    ...
    public List<Wall> CreateWalls()
    {
      ...
      for (int i = 0; i <= 3; i++)
      {
        ...
        Line baseCurve = m_rvtApp.Create.NewLineBound(pts[i], pts[i + 1]);
        //' create a wall using the one of overloaded methods. 
        Wall aWall = m_rvtDoc.Create.NewWall(baseCurve, level1, isStructural);
    ...
    public void AddDoor(Wall hostWall)
    {
      ...
      FamilySymbol doorType = (FamilySymbol)ElementFiltering.FindFamilyType(m_rvtDoc, typeof(FamilySymbol), doorFamilyName, doorTypeName, BuiltInCategory.OST_Doors);
      if (doorType == null)
      {
      ...
      Level level1 = (Level)m_rvtDoc.get_Element(idLevel1);
    ...
    public void AddWindow(Wall hostWall)
    {
      ...
      //' get the door type to use. 
      FamilySymbol windowType = (FamilySymbol)ElementFiltering.FindFamilyType(m_rvtDoc, typeof(FamilySymbol), windowFamilyName, windowTypeName, BuiltInCategory.OST_Windows);
      if (windowType == null)
      {
    ...
    public void addRoof(List<Wall> walls)
    {
      ...
      //' find the roof type 
      RoofType roofType = (RoofType)ElementFiltering.FindFamilyType(m_rvtDoc, typeof(RoofType), roofFamilyName, roofTypeName, null);
      if (roofType == null)
      {
      ...
      Level level2 = (Level)m_rvtDoc.get_Element(idLevel2);
