...
namespace Revit.SDK.Samples.FindColumns.CS
{
    ...
    public class Command : IExternalCommand
    {
        ...
        {
            ReferenceArray intersectedReferences = m_doc.FindReferencesByDirection(rayStart, rayDirection, m_view3D);
        ...
        private void FindColumnsWithin(ReferenceArray references, double proximity, Wall wall)
        {
            foreach (Reference reference in references)
            {
                // Exclude items too far from the start point.
                if (reference.ProximityParameter < proximity)
                {
                    Autodesk.Revit.DB.Element referenceElement = reference.Element;
    ...
    [Autodesk.Revit.Attributes.Transaction(Autodesk.Revit.Attributes.TransactionMode.ReadOnly)]
    [Autodesk.Revit.Attributes.Regeneration(Autodesk.Revit.Attributes.RegenerationOption.Manual)]
    public class FindByFilter : IExternalCommand
    {
        /// <summary>
        /// revit application
        /// </summary>
        ...
        public Result Execute(ExternalCommandData revit, ref string message, ElementSet elements)
        {
            ...
            // Find all walls in the document, and process those whose location curve aligns with the X or Y axis.
            else
            {
                FilteredElementCollector collector = new FilteredElementCollector(m_doc);
                FilteredElementIterator iter = collector.OfClass(typeof(Wall)).GetElementIterator();
                ...
                while(iter.MoveNext())
                {
                    ...
                    if (curve is Line)
                    {
                        ...
                        XYZ direction = line.get_EndPoint(1) - line.get_EndPoint(0);
                        double angleToX = direction.AngleTo(XYZ.BasisX);
                        double angleToY = direction.AngleTo(XYZ.BasisY);

                        if (Math.Abs(Math.Sin(angleToX)) > 1e-9 && Math.Abs(Math.Sin(angleToY)) > 1e-9)
                        {
                            continue;
                        }
                    }
            ...
            foreach (Wall wall in wallsToCheck)
            {
                FindEmbeddedColumnsInWall(wall);
            }
        ...
        void FindEmbeddedColumnsInWall(Wall wall)
        {
            BoundingBoxXYZ bbox = wall.get_BoundingBox(null);

            Outline outline = new Outline(bbox.get_Bounds(0), bbox.get_Bounds(1));

            FilteredElementCollector columnCollector = new FilteredElementCollector(m_doc);

            LogicalOrFilter categoryFilter = new LogicalOrFilter(new ElementCategoryFilter(BuiltInCategory.OST_Columns), 
                new ElementCategoryFilter(BuiltInCategory.OST_StructuralColumns));

            columnCollector.WhereElementIsNotElementType().WherePasses(categoryFilter);
            columnCollector.WherePasses(new BoundingBoxIntersectsFilter(outline));

            ICollection<ElementId> columnIds = columnCollector.ToElementIds();

            foreach (ElementId id in columnIds)
            {
                m_allColumnsOnWalls.Add(id);
            }
        }
