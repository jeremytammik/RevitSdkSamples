...
namespace RevitIntroCS
{
  ...
  public class DBElement : IExternalCommand
  {
      ...
      ElementSet elements )
    {
      ...
      // (1) select an object on a screen. (We'll come back to 
      // the selection in the UI Lab later.)

      Reference r = rvtUIDoc.Selection.PickObject( 
      ...
      // Class, Category and if an element is ElementType or not.

      ShowBasicElementInfo( elem );

      // check the basic info of the element type
      //Dim elemTypeId As ElementId = elem.GetTypeId
      //Dim elemType As ElementType = _rvtDoc.Element(elemTypeId)
      ...
      // Now look at other properties - important ones are 
      // parameters, locations and geometry.
      ...
      ElementType elemType = _rvtDoc.get_Element( elemTypeId ) as ElementType;
      ShowParameters( elemType, "Type Parameters: " );

      // we saw a set or parameters for a given element or element type.
      // how can we access each parameter? For example, how can we get 
      // the value of "length" information?
    ...
    public void ShowBasicElementInfo( Element elem )
    {
      ...
      ElementId elemTypeId = elem.GetTypeId();
      ElementType elemType = ( ElementType ) _rvtDoc.get_Element( elemTypeId );
      s = s + "Its ElementType:" + "\n";
    ...
    public static string ParameterToString( Parameter param )
    {
      ...
      // get the parameter value depending on its storage type
    ...
    public void RetrieveParameter( 
      Element elem, 
      [System.Runtime.InteropServices.DefaultParameterValueAttribute( "" )] string header )
    {
      string s = header + "\n\n";

      // as an experiment, let's pick up some arbitrary parameters.
      // note: most instances have this parameter
      ...
      if( param != null )
      {
        s = s + "Comments (by BuiltInParameter) = " 
          + ParameterToString( param ) + "\n";
      }

      // (2) by name. (Mark - most instances have this parameter.) 
      // if you use this method, it will be language specific.
      ...
      // the following should be in most types

      param = elem.get_Parameter( BuiltInParameter.ALL_MODEL_TYPE_COMMENTS );
      if( param != null )
      {
        s = s + "Type Comments (by BuiltInParameter) = " 
          + ParameterToString( param ) + "\n";
      }
      ...
      if( param != null )
      {
        s = s + "Fire Rating (by Name) = " 
          + ParameterToString( param ) + "\n";
      }

      // using the BuiltInParameter, you can sometimes access 
      // one that is not in the parameters set.
      ...
      if( param != null )
      {
        s = s + "SYMBOL_FAMILY_AND_TYPE_NAMES_PARAM (only by BuiltInParameter) = " 
          + ParameterToString( param ) + "\n";
      }
      ...
      if( param != null )
      {
        s = s + "SYMBOL_FAMILY_NAME_PARAM (only by BuiltInParameter) = " 
          + ParameterToString( param ) + "\n";
      }
      TaskDialog.Show( "Revit Intro Lab", s );
    }
    ...
    // Helper Function: return XYZ in a string form.
    ...
    public void ShowGeometry( Element elem )
    {
      ...
      Options opt = _rvtApp.Create.NewGeometryOptions();
