...
Public Class RvtCmd_CreateColumn_v2
    ...
    Public Function Execute(ByVal commandData As ExternalCommandData, _
        ...
        'Dim res As TaskDialogResult '' result of task dialog 
        ...
        'res = TaskDialog.Show("Family API", "Do you want to continue?", (TaskDialogCommonButtons.Yes Or TaskDialogCommonButtons.No))
        'If res = TaskDialogResult.No Then
    ...
    Public Sub New(ByVal commandData)

        init(commandData)

    End Sub

    Sub init(ByVal commandData As ExternalCommandData)
    ...
    Function ValidateDocument() As Boolean
        ...
        ''  our command works in the context of family editor only 
        If Not m_rvtDoc.IsFamilyDocument Then
            TaskDialog.Show("Family API", "This command works only in the family editor.")
            Return False
        End If

        ''  check if we have a right template 
        Dim ownerFamily As Family = m_rvtDoc.OwnerFamily
        If ownerFamily Is Nothing Then
            TaskDialog.Show("Family API", "This document does not have Owner Family.")
            Return False
        End If
        ...
        ''  check the family category of this document         
        ''  we assume that we want to create a column. Hard-coding for simplicity. 
        Dim catColumn As Category = m_rvtDoc.Settings.Categories.Item(BuiltInCategory.OST_Columns)
        If Not ownerFamily.FamilyCategory.Id.Equals(catColumn.Id) Then
            TaskDialog.Show("Family API", "The category of this document does not match the context of this commands. Please open Metric Column.rft")
        End If
...
#Region "Geometry Solid"
    ...
    '' ============================================
    ''   (7.1.1) create a simple solid by extrusion with L-shape profile
    '' ============================================
    ...
    '' ============================================
    ''   (7.1.1.1) create a simple L-shaped profile
    '' ============================================
    ...
    '' ============================================
    ''   (7.1.3) set the visibility of the solid not to show in coarse
    '' ============================================
    ...
    Sub addMaterials(ByVal pSolid As Extrusion)
        ...
        ''  (2b) get a parameter for material finish
        ''
...
#Region "Geometry Model Line"
    ...
    '' ============================================
    ''   add alignments to the model line 
    '' ============================================
...
#Region "Geometry Symbolic Line"
    ...
    '' ============================================
    ''   (7.3) add geometry symbolic line 
    ''         create simple line objects to be displayed in coarse level
    '' ============================================
    ...
    Sub AddAlignments(ByVal pSymbolicLines As List(Of SymbolicCurve))
        ...
        ''  To Do. we leave this as an exercise.  
...
#Region "Step By Step Commands"

<Transaction(TransactionMode.Automatic)> _
<Regeneration(RegenerationOption.Manual)> _
Public Class RvtCmd_CreateColumn_Validate
    Implements IExternalCommand
    ...
    Public Function Execute(ByVal commandData As ExternalCommandData, _
        ...
        Dim myCreateColumn As New RvtCmd_CreateColumn_v2(commandData)
        'myCreateColumn.init(commandData)

        If Not myCreateColumn.ValidateDocument() Then
            Return Result.Failed
        End If
...
<Transaction(TransactionMode.Automatic)> _
<Regeneration(RegenerationOption.Manual)> _
Public Class RvtCmd_CreateColumn_Reference
    Implements IExternalCommand
    ...
    Public Function Execute(ByVal commandData As ExternalCommandData, _
        ...
        myCreateColumn.AddReferencePlanes()
...
<Transaction(TransactionMode.Automatic)> _
<Regeneration(RegenerationOption.Manual)> _
Public Class RvtCmd_CreateColumn_Parameter
    Implements IExternalCommand
...
<Transaction(TransactionMode.Automatic)> _
<Regeneration(RegenerationOption.Manual)> _
Public Class RvtCmd_CreateColumn_Dimension
    Implements IExternalCommand
...
<Transaction(TransactionMode.Automatic)> _
<Regeneration(RegenerationOption.Manual)> _
Public Class RvtCmd_CreateColumn_Types
    Implements IExternalCommand
...
<Transaction(TransactionMode.Automatic)> _
<Regeneration(RegenerationOption.Manual)> _
Public Class RvtCmd_CreateColumn_Solid
    Implements IExternalCommand
    ...
    Public Function Execute(ByVal commandData As ExternalCommandData, _
        ...
        myCreateColumn.AddGeometrySolid()
...
<Transaction(TransactionMode.Automatic)> _
<Regeneration(RegenerationOption.Manual)> _
Public Class RvtCmd_CreateColumn_ModelLine
    Implements IExternalCommand
    ...
    Public Function Execute(ByVal commandData As ExternalCommandData, _
        ...
        myCreateColumn.AddGeometryModelLine()
...
<Transaction(TransactionMode.Automatic)> _
<Regeneration(RegenerationOption.Manual)> _
Public Class RvtCmd_CreateColumn_SymbolicLine
    Implements IExternalCommand
    ...
    Public Function Execute(ByVal commandData As ExternalCommandData, _
        ...
        myCreateColumn.AddGeometrySymbolicLines()
...
<Transaction(TransactionMode.Automatic)> _
<Regeneration(RegenerationOption.Manual)> _
Public Class RvtCmd_CreateColumnInstances
    Implements IExternalCommand
    ...
    Sub AddColumnInstances()

        Dim typeCollector = New FilteredElementCollector(m_rvtDoc)
        typeCollector.OfClass(GetType(FamilySymbol))
        typeCollector.OfCategory(BuiltInCategory.OST_Columns)
        Dim types As IList(Of Element) = typeCollector.ToElements

        Dim level1 As Level = Utils.FindElement(m_rvtDoc, GetType(Level), "Level 1")

        Dim x As Double = 0.0
        'Dim pt As New XYZ(x, 0.0, 0.0)

        For Each aType As FamilySymbol In types

            Dim pt As New XYZ(x, 0.0, 0.0)
            'AddColumnInstance(aType)
            Dim aColumn As FamilyInstance = m_rvtDoc.Create.NewFamilyInstance( _
                pt, aType, level1, StructuralType.NonStructural)
            x += 3.0
