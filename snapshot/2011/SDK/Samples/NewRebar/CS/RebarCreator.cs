...
namespace Revit.SDK.Samples.NewRebar.CS
{
    ...
    class RebarCreator
    {
        ...
        /// </summary>
        Autodesk.Revit.UI.UIApplication m_rvtApp;
        ...
        private void Assert()
        {
            ...
            foreach (Autodesk.Revit.DB.Element elem in m_rvtApp.ActiveUIDocument.Selection.Elements)
            {
            ...
            FilteredElementCollector collector = new FilteredElementCollector(m_rvtApp.ActiveUIDocument.Document, selectedIds);
            ...
            FilteredElementIterator itor = (new FilteredElementCollector(m_rvtApp.ActiveUIDocument.Document)).OfClass(typeof(Rebar)).GetElementIterator();
            while (itor.MoveNext())
            {
                Rebar rebar = itor.Current as Rebar;
                if (null != rebar && null != rebar.Host && rebar.Host.Id.IntegerValue == m_rebarHost.Id.IntegerValue)
                {
        ...
        public void Execute()
        {
            using (NewRebarForm form = new NewRebarForm(m_rvtApp))
            {
                if (DialogResult.OK == form.ShowDialog())
                {
                    ...
                    m_createdRebar = m_rvtApp.ActiveUIDocument.Document.Create.
        ...
        private void LayoutRebar()
        {
            ...
            RebarShapeDefinitionByArc arcDef = 
                m_createdRebar.RebarShape.Definition as RebarShapeDefinitionByArc;
            if (arcDef != null && arcDef.Type == RebarShapeDefinitionByArcType.Spiral)
            {                
                m_createdRebar.ScaleToBoxForSpiral(origin, xVec, yVec, 10.0);
