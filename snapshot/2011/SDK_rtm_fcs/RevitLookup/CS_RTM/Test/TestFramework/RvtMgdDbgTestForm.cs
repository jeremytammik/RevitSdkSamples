...
namespace RvtMgdDbg.Test
{
   ...
   public class TestForm : System.Windows.Forms.Form
   {
      ...
      private RvtMgdDbgTestFuncInfo m_pickedTestFunc = null;
      //private Rvt.Utils.Controls.TreeViewWithSearch m_treeViewWithSearch;

      private RvtMgdDbg.Utils.ListViewColumnSorter m_colSorter;
      ...
      TestForm(ArrayList testFuncs)
      {
         ...
         m_colSorter = new RvtMgdDbg.Utils.ListViewColumnSorter();
         m_lvData.ListViewItemSorter = m_colSorter;

         m_treeView.BeginUpdate();

         // sort the TestFuncInfo objects into buckets
         foreach (RvtMgdDbgTestFuncs testFuncGroup in testFuncs)
         {
            foreach (RvtMgdDbgTestFuncInfo testFuncInfo in testFuncGroup.m_testFuncs)
            {
      ...
      BuildTree()
      {
         ...
         RvtMgdDbgTestFuncInfo tmpTestFuncInfo = null;
         ...
         for (int i = 0; i < len; i++)
         {
            ...
            tmpTestFuncInfo = (RvtMgdDbgTestFuncInfo)tmpBucket[0];
      ...
      GetTests(Type type, bool includeBaseType)
      {
         ...
         {
            foreach (RvtMgdDbgTestFuncInfo tempInfo in tmpBucket)
            {
      ...
      private void
      AddFuncToBucket(RvtMgdDbgTestFuncInfo newTestFuncInfo)
      {
         RvtMgdDbgTestFuncInfo tempFuncInfo = null;
         ArrayList tmpBucket = null;
         ...
         {
            tmpBucket = (ArrayList)m_funcBuckets[i];
            tempFuncInfo = (RvtMgdDbgTestFuncInfo)tmpBucket[0]; // get the first element to compare against
      ...
      DisplayCurrent()
      {
         ...
         for (int i = 0; i < len; i++)
         {
            RvtMgdDbgTestFuncInfo tmpTestFuncInfo = (RvtMgdDbgTestFuncInfo)m_curFuncBucket[i];
      ...
      ItemSelected(object sender, System.EventArgs e)
      {
         ...
         {
            m_pickedTestFunc = (RvtMgdDbgTestFuncInfo)m_lvData.SelectedItems[0].Tag;
