    ...
    public class RoomsData
    {
        ...
        public DataTable GenRoomsDataTable(Level level)
        {
            ...
            foreach (Room room in m_rooms)
            {
                ...
                if ((null == level) || (room.Level != null && room.Level.Id.IntegerValue == level.Id.IntegerValue))
                {
        ...
        private void GetAllRooms(Document activeDoc)
        {
			...
			FilteredElementIterator roomIter = collector.WherePasses(filter).GetElementIterator();
			roomIter.Reset();
			while (roomIter.MoveNext())
            {
				...
				Room tmpRoom = roomIter.Current as Room;
