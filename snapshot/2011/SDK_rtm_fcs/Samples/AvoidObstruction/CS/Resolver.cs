    ...
    class Resolver
    {
        ...
        private void Filter(Pipe pipe, List<Reference> refs)
        {
            for (int i = refs.Count - 1; i >= 0; i--)
            {
                ...
                if (cur.Element.Id.IntegerValue == pipe.Id.IntegerValue ||
                (!(cur.Element is Pipe) && !(cur.Element is Duct) &&
                cur.Element.Category.Id.IntegerValue != (int)BuiltInCategory.OST_StructuralFraming))
                {
                    refs.RemoveAt(i);
                }
            }
        ...
        private Connector FindConnectedTo(Pipe pipe, Autodesk.Revit.DB.XYZ  conXYZ)
        {
            ...
            foreach (Connector conn in connSet)
            {
                if (conn.Owner.Id.IntegerValue != pipe.Id.IntegerValue &&
                    conn.ConnectorType == ConnectorType.EndConn)
                {
                    return conn;
                }
            }
