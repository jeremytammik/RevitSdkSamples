...
namespace Revit.SDK.Samples.Truss.CS
{
    ...
    public partial class TrussForm : System.Windows.Forms.Form
    {
        ...
        public void DataInitialize()
        {
            ...
            if (null == m_activeDocument.Document.TrussTypes ||
                0 == m_activeDocument.Document.TrussTypes.Size)
            {
                MessageBox.Show("Please load at least one truss type into your project.");
                this.Close();
            }

            foreach (TrussType trussType in m_activeDocument.Document.TrussTypes)
            {
                if (null == trussType)
                {
                    continue;
        ...
        private void GetBeamTypes()
        {
            m_beamTypes = from elem in
                          new FilteredElementCollector(m_activeDocument.Document).OfClass(typeof(FamilySymbol)).ToElements()
                          let type = elem as FamilySymbol
                          where type!=null && type.Category!=null  && type.Category.Name == "Structural Framing"
                          select type;
