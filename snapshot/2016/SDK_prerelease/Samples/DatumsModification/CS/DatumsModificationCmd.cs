                   ...
                   if (settingForm.ShowDialog() == DialogResult.OK)
                   {
                      using (Transaction tran = new Transaction(document,"StyleModification"))
                      {
                         ...
                         foreach (ElementId datumRef in datums)
                         {
                            ...
                            if (showLeftBubble)
                               datum.ShowBubble(DatumEnds.End0, view);
                            else
                               datum.HideBubble(DatumEnds.End0, view);
                            if (showRightBubble)
                               datum.ShowBubble(DatumEnds.End1, view);
                            else
                               datum.HideBubble(DatumEnds.End1, view);
                            if (changeLeftEnd2D)
             ...
             using (AlignmentSetting settingForm = new AlignmentSetting())
             {
                if (settingForm.ShowDialog() == DialogResult.OK)
                {
                   ...
                   using (Transaction tran = new Transaction(document, "DatumAlignment"))
                   {
                      ...
                      {
                         Curve curve = datum.GetCurvesInView(datum.GetDatumExtentType(DatumEnds.End0, view),view).ElementAt(0);
                         XYZ direct = (curve as Line).Direction;
                         Curve newCurve = CalculateCurve(curve,baseLine,baseDirect);
                         datum.SetCurveInView(datum.GetDatumExtentType(DatumEnds.End0, view), view, newCurve);                         
                      }
                      tran.Commit();
                   }
                }
    ...
    public class DatumPropagation : IExternalCommand
    {
           ...
           , ref string message, ElementSet elements)
       {
          try
          {
             ...
             DatumPlane datum = document.GetElement(datumRef) as DatumPlane;
             IList<ElementId> viewList = datum.GetPropagationViews(view);
             ...
             using (PropogateSetting settingForm = new PropogateSetting())
             {
                if (settingForm.ShowDialog() == DialogResult.OK)
                {
                   List<ElementId> pViewList = new List<ElementId>();
