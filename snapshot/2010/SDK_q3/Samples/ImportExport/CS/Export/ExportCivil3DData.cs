...
namespace Revit.SDK.Samples.ImportExport.CS
{
    ...
    public class ExportCivil3DData : ExportData
    {
        ...
        /// Revit application
        /// </summary>
        private Autodesk.Revit.Application m_application;
        /// <summary>
        /// Revit document
        /// </summary>
        ...
        /// </summary>
        private List<Element> m_view3DList;
        /// <summary>
        ...
        /// </summary>
        private List<Element> m_propertyLineList;
        /// <summary>
        ...
        {
            Autodesk.Revit.TypeFilter typeFilter = m_application.Create.Filter.NewTypeFilter(typeof(View3D), true);
            m_view3DList = new List<Element>();
            m_document.get_Elements(typeFilter, m_view3DList);

            if (m_view3DList.Count == 0)
            {
                View3D view3D = m_document.Create.NewView3D(new Autodesk.Revit.Geometry.XYZ(0, 0, 1));
        ...
        {
            Autodesk.Revit.TypeFilter typeFilter = m_application.Create.Filter.NewTypeFilter(typeof(PropertyLine), true);
            m_propertyLineList = new List<Element>();
            m_document.get_Elements(typeFilter, m_propertyLineList);
        }
        ...
        {
            Autodesk.Revit.TypeFilter typeFilter = m_application.Create.Filter.NewTypeFilter(typeof(ViewPlan), true);
            IList<Element> viewPlanList = new List<Element>();
            m_document.get_Elements(typeFilter, viewPlanList);
            m_grossAreaPlanList = new List<ViewPlan>();
            foreach (ViewPlan vp in viewPlanList)
            {
                if (vp.AreaScheme != null && vp.AreaScheme.IsGrossBuildingArea == true)
                {
            ...
            {
                MessageBox.Show("The document does not contain any gross area plan.", "No Gross Area Plan",
