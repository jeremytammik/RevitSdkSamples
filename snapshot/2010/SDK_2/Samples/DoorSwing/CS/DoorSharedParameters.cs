...
namespace Revit.SDK.Samples.DoorSwing.CS
{
   ...
   public static class DoorSharedParameters
   {
      ...
      public static void AddSharedParameters(Application app)
      {
         ...
         Category doorCategory = app.ActiveDocument.Settings.Categories.
         ...
         DefinitionFile defFile = AccessOrCreateSharedParameterFile(app);
         if (null == defFile)
         {
            return;
         }
         ...
         // for "BasalOpening"
         if (!AlreadyAddedSharedParameter(app.ActiveDocument, "BasalOpening", BuiltInCategory.OST_Doors))
         {
         ...
         // for "InstanceOpening"
         if (!AlreadyAddedSharedParameter(app.ActiveDocument, "InstanceOpening", BuiltInCategory.OST_Doors))
         {
            Definition instanceOpening = defGroup.Definitions.get_Item("InstanceOpening");
            
            if (null == instanceOpening)
            {
               instanceOpening = defGroup.Definitions.Create("InstanceOpening", ParameterType.Text, false);
            }
         ...
         // for "Internal Door"
         if (!AlreadyAddedSharedParameter(app.ActiveDocument, "Internal Door", BuiltInCategory.OST_Doors))
         {
            Definition internalDoorFlag = defGroup.Definitions.get_Item("Internal Door");
            
            if (null == internalDoorFlag)
            {
               internalDoorFlag = defGroup.Definitions.Create("Internal Door", ParameterType.YesNo, false);
            }
      ...
      private static DefinitionFile AccessOrCreateSharedParameterFile(Application app)
      {
         ...
         app.Options.SharedParametersFilename = sharedParameterFilePath;
         ...
         try
         {
            ...
            while (bindingMapIter.MoveNext())
            {
               if (bindingMapIter.Key.Name.Equals(paraName))
               {
                  ...
                  {
                     if (category.Id.Value.Equals((int)boundCategory))
                     {
                        return true;
                     }
                  }
