...
namespace Revit.SDK.Samples.Reinforcement.CS
{
    ...
    public class BeamGeometrySupport : GeometrySupport
    {
        ...
        public RebarGeometry GetTopRebar(TopRebarLocation location)
        {
            ...
            curves.Append(Line.get_Bound(startPoint, endPoint));
        ...
        public RebarGeometry GetTransverseRebar(TransverseRebarLocation location, double spacing)
        {
            ...
            List<XYZ> translatedPoints = new List<XYZ>();
            foreach (XYZ point in movedPoints)
            {
                translatedPoints.Add(GeomUtil.OffsetPoint(point, m_drivingVector, curveOffset));
            }

            CurveArray curves = new CurveArray();
            XYZ first = translatedPoints[0];
            XYZ second = translatedPoints[1];
            XYZ third = translatedPoints[2];
            XYZ fourth = translatedPoints[3];

            curves.Append(Line.get_Bound(first, second));
            curves.Append(Line.get_Bound(second, fourth));
            curves.Append(Line.get_Bound(fourth, third));
            curves.Append(Line.get_Bound(third, first));
