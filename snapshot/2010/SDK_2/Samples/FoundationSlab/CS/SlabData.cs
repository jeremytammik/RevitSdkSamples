...
namespace Revit.SDK.Samples.FoundationSlab.CS
{
    ...
    public class SlabData
    {
        ...
        public SlabData(Application revit)
        {
            m_revit = revit;
            CreApp = m_revit.Create;
        ...
        public bool CreateFoundationSlabs()
        {
            ...
            foreach (RegularSlab slab in m_allBaseSlabList)
            {
                ...
                Floor foundationSlab = m_revit.ActiveDocument.Create.NewFoundationSlab(
                ...
                ElementId deleteSlabId = slab.Id;
                m_revit.ActiveDocument.Delete(ref deleteSlabId);
            }
            return true;
        }
        ...
        {
            ElementIterator iterator = m_revit.ActiveDocument.Elements;
            iterator.Reset();
        ...
        private bool GetAllBaseSlabs()
        {
            ...
            {
                if (floor.Level.Id.Value == m_levelList.Values[0].Id.Value)
                {
        ...
        {         
            ...
            {
                AnalyticalModelFloor analyticalModel = floor.AnalyticalModel as AnalyticalModelFloor;
                return analyticalModel.Curves;
            }

            // Nonstructural floor's profile can be formed through it's Geometry.
            Options aOptions = m_revit.Create.NewGeometryOptions();
            Autodesk.Revit.Geometry.Element aElementOfGeometry = floor.get_Geometry(aOptions);
            ...
            foreach (GeometryObject o in geometryObjects)
            {
                ...
                for (int i = 0; i < (edges.Size) / 3; i++)
                {
                    ...
                    XYZArray xyzArray = edge.Tessellate();  // A set of points.
                    for (int j = 0; j < (xyzArray.Size - 1); j++)
                    {
                        XYZ startPoint = xyzArray.get_Item(j);
                        XYZ endPoint = xyzArray.get_Item(j + 1);
