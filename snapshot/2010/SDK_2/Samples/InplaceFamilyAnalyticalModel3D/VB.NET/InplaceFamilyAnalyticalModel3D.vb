...
Public Class Command
    ...
    Public Function Execute(ByVal commandData As Autodesk.Revit.ExternalCommandData, ByRef message As String, ByVal elements As Autodesk.Revit.ElementSet) As Autodesk.Revit.IExternalCommand.Result Implements Autodesk.Revit.IExternalCommand.Execute
        Dim returnCode As IExternalCommand.Result = IExternalCommand.Result.Cancelled

        ' Create the log file.
        Dim actullyLogFile As String = Path.Combine(Path.GetDirectoryName(Me.GetType().Assembly.Location), "InplaceFamilyAnalyticalModel3D.log")

        'if file already existed, delete it.
        If File.Exists(actullyLogFile) Then
            File.Delete(actullyLogFile)
        End If

        Dim textListener As TraceListener = New TextWriterTraceListener(actullyLogFile)
        ...
        Try
            ...
            For Each element In doc.Selection.Elements
                ...
                If TypeOf element Is Autodesk.Revit.Elements.FamilyInstance Then
                    ...
                    If Not (analyticalModel Is Nothing) Then
                        ...
                        Else
                            Trace.WriteLine("We should select analytical model 3D family instance, but this familyInstance.AnalyticalModel type is " + analyticalModel.GetType().Name)
                            Continue For
                        End If
            ...
            returnCode = IExternalCommand.Result.Succeeded
        Catch ex As Exception
            message = ex.ToString()
            Trace.WriteLine(message)
            returnCode = IExternalCommand.Result.Failed
        Finally
            ' Close the log file
            Trace.Flush()
            textListener.Close()
    ...
    Sub DumpFamilyInstance(ByVal familyInstance As Autodesk.Revit.Elements.FamilyInstance)
        ...
        Trace.WriteLine("Family Name : " & familyInstance.Symbol.Family.Name)
        Trace.WriteLine("Family Symbol Name : " & familyInstance.Symbol.Name)
        Trace.WriteLine("Family Instance Name : " & familyInstance.Name)
    End Sub
    ...
    Sub DumpAnalyticalModel3D(ByVal analyticalModel As Autodesk.Revit.Structural.AnalyticalModel3D)
        ...
        For Each curve In analyticalModel.Curves
            ...
            For Each point In points
                Trace.WriteLine(point.X.ToString() + ", " + point.Y.ToString() + ", " + point.Z.ToString())
            Next
