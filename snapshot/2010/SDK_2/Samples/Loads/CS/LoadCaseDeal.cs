...
namespace Revit.SDK.Samples.Loads.CS
{
    ...
    {
        // Private Members
        Autodesk.Revit.Application m_revit; // Store the reference of revit application
        Loads m_dataBuffer;                 
        ...
        public void PrepareData()
        {
            ...
            Categories categories = m_revit.ActiveDocument.Settings.Categories;
        ...
        private bool CreateLoadNatures()
        {
            ...
            try
            {
                for (int i = 0; i < m_newLoadNaturesName.Count; i++)
                {
                    ...
                    m_revit.ActiveDocument.Create.NewLoadNature(temp);
                }
        ...
        public bool AddLoadNature(int index)
        {
            ...
            try
            {
                LoadNature newLoadNature = m_revit.ActiveDocument.Create.NewLoadNature(natureName);
        ...
        public bool DuplicateLoadCase(int index)
        {
            ...
            ElementId categoryId = myLoadCase.LoadCasesCategoryId;
            ElementId natureId = myLoadCase.LoadCasesNatureId;

            ElementIterator iMoreElement = m_revit.ActiveDocument.Elements;
            iMoreElement.Reset();
            while (iMoreElement.MoveNext())
            {
                Element e = iMoreElement.Current as Element;
                if (natureId.Value == e.Id.Value)
                {
                    caseNature = e as LoadNature;
                    break;
                }
            }
            ...
            while (iter.MoveNext())
            {
                ...
                if (null != tempC && (categoryId.Value == tempC.Id.Value))
                {
                    caseCategory = tempC;
                    break;
                }
            ...
            //check if lack of the informations
            ...
            try
            {
                LoadCase newLoadCase = m_revit.ActiveDocument.Create.NewLoadCase(caseName, caseNature, caseCategory);
                if (null == newLoadCase)
                {
