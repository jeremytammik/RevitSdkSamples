...
using GXYZ = Autodesk.Revit.Geometry.XYZ;
...
namespace Revit.SDK.Samples.AutoRoute.CS
{
    ...
    public class Command : IExternalCommand
    {
            ...
            ElementSet elements)
        {
            ...
            try
            {
                ...
                ElementId[] ids = new ElementId[3];
                ids[0].Value = 378728;
                ids[1].Value = 378707;
                ids[2].Value = 378716;
                FamilyInstance[] instances = new FamilyInstance[3];
                Autodesk.Revit.Geometry.BoundingBoxXYZ[] boxes = new Autodesk.Revit.Geometry.BoundingBoxXYZ[3];
                Connector[] conns = new Connector[3];
                ConnectorSetIterator csi = null;
                for (int i = 0; i < ids.Length; ++i)
                {
                    Element element = m_document.get_Element(ref ids[i]);
                    if (null == element)
                    {
                        message = "Element " + ids[i].Value + " can't be found.";
                        return IExternalCommand.Result.Failed;
                    }
                    instances[i] = element as FamilyInstance;
                    csi = ConnectorInfo.GetConnectors(ids[i].Value).ForwardIterator();
                    csi.MoveNext();
                ...
                csi = ConnectorInfo.GetConnectors(ids[0].Value).ForwardIterator();
                while (csi.MoveNext())
                {
                ...
                ElementId ductTypeId = new ElementId();
                ductTypeId.Value = 139191;
                dtRectangle = m_document.get_Element(ref ductTypeId) as DuctType;
                ...
                if (0 == connectorDirection.Distance(new GXYZ(-1, 0, 0)))
                {
                ...
                else if (0 == connectorDirection.Distance(new GXYZ(1, 0, 0)))
                {
                    points.Add(new GXYZ(conns[0].Origin.X + min1FittingLength, conns[0].Origin.Y, conns[0].Origin.Z));
                    points.Add(new GXYZ(conns[0].Origin.X + min2FittingsLength, conns[0].Origin.Y, conns[0].Origin.Z + min1FittingLength));
                    points.Add(new GXYZ(conns[0].Origin.X + min2FittingsLength, conns[0].Origin.Y, maxZ + verticalTrunkOffset - min1FittingLength));
                }
                else if (0 == connectorDirection.Distance(new GXYZ(0, -1, 0)))
                {
                ...
                connectors.Add(ConnectorInfo.GetConnector(ducts[0].Id.Value, points[0]));
                connectors.Add(ConnectorInfo.GetConnector(ducts[1].Id.Value, points[1]));
                connectors.Add(ConnectorInfo.GetConnector(ducts[1].Id.Value, points[2]));
                ...
                baseConnectors.Add(ConnectorInfo.GetConnector(ducts[0].Id.Value, points[0]));
                baseConnectors.Add(ConnectorInfo.GetConnector(ducts[1].Id.Value, points[1]));
            ...
            catch (Exception ex)
            {
                ...
                message = ex.Message;
                retRes = IExternalCommand.Result.Failed;
            }
            finally
            {
                Trace.Flush();
        ...
        private List<Connector> CreateDuct(GXYZ point1, GXYZ point2)
        {
            ...
            connectors.Add(ConnectorInfo.GetConnector(duct.Id.Value, point1));
            connectors.Add(ConnectorInfo.GetConnector(duct.Id.Value, point2));
        ...
        public class ConnectorInfo
        {
            ...
            public static Connector GetConnector(int ownerId, GXYZ connectorOrigin)
            {
                ...
                {
                    if (conn.ConnectorType == ConnectorType.LogicalConn)
                        continue;
                    if (conn.Origin.AlmostEqual(connectorOrigin))
                        return conn;
                }
                return null;
            }
            ...
            {
                ElementId elementId;
                elementId.Value = ownerId;
                Element element = m_document.get_Element(ref elementId);
            ...
            public static ConnectorSet GetConnectors(Autodesk.Revit.Element element)
            {
            ...
            public static Connector[] FindConnectors(ConnectorManager connMgr, GXYZ pnt1, GXYZ pnt2)
            {
                ...
                {
                    if (conn.Origin.AlmostEqual(pnt1))
                    {
                        result[0] = conn;
                    }
                    else if (conn.Origin.AlmostEqual(pnt2))
                    {
                        result[1] = conn;
                    }
                }
    ...
    public class LogUtility
    {
        ...
        public static void WriteElement(Element element, bool writeId)
        {
            ...
            int elementId = element.Id.Value;
            int familyId = element.get_Parameter(BuiltInParameter.ELEM_FAMILY_PARAM).AsElementId().Value;
            ...
            BuiltInCategory category = (BuiltInCategory)(element.get_Parameter(BuiltInParameter.ELEM_CATEGORY_PARAM).AsElementId().Value);
        ...
        public static T GetElement<T>(Document document, int eid) where T : Autodesk.Revit.Element
        {
            ElementId elementId;
            elementId.Value = eid;
            return document.get_Element(ref elementId) as T;
        }
        ...
        {
            Trace.WriteLine("IsDefaultSystem: " + system.IsDefaultSystem);
            Trace.WriteLine("+BaseEquipment");
            Trace.Indent();
        ...
        {
            Trace.WriteLine("IsReadOnly: " + connectorManager.IsReadOnly.ToString());
        ...
        public static string GetConnectorId(Connector connector)
        {
            ...
            int ownerId = connector.Owner.Id.Value;
            string systemId = InvalidString;
            try { systemId = connector.MEPSystem.Id.Value.ToString(); }
            catch { }
            object connType = InvalidString;
            object connDirection = InvalidString;
            object connShape = InvalidString;
        ...
        private static string GetShapeInfo(Connector conn)
        {
            switch (conn.Shape)
            {
                case ConnectorProfileType.InvalidProfile:
                    break;
        ...
        private static object GetLocation(Connector conn)
        {
            ...
            Autodesk.Revit.Geometry.XYZ origin = conn.Origin;
