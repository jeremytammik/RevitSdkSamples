...
namespace Revit.SDK.Samples.WindowWizard.CS
{
    ...
    class DoubleHungWinCreation : WindowCreation
    {
        ...
        /// </summary>
        private Autodesk.Revit.FamilyManager m_familyManager;
        ...
        /// </summary>
        Autodesk.Revit.Elements.View m_rightView;
        ...
        public DoubleHungWinCreation(WizardParameter para, ExternalCommandData commandData) : base(para)
        {
            ...
            {
                case Autodesk.Revit.Enums.DisplayUnit.METRIC:
                    para.Validator.IsMetric = true;
                    break;
                case Autodesk.Revit.Enums.DisplayUnit.IMPERIAL:
        ...
        public override void CreateSash()
        {
            ...
            Autodesk.Revit.Elements.View exteriorView = Utility.GetViewByName("Exterior", m_application, m_document);
            m_document.BeginTransaction();

            //add a middle reference plane between the top referenceplane and sill referenceplane
            CreateRefPlane refPlaneCreator = new CreateRefPlane();            
            ReferencePlane middlePlane=refPlaneCreator.Create(m_document, m_topPlane, exteriorView, new XYZ(0, 0, -m_height / 2), new XYZ(0, -1, 0), "tempmiddle");
            ...
            ElementId id;
            id.Value = m_sashMatID;
            sash1.ParametersMap.get_Item("Material").Set(ref id);
            sash2.ParametersMap.get_Item("Material").Set(ref id);
            m_document.EndTransaction();
        }
        ...
        public override void CreateGlass()
        {
            ...
            ElementId id;
            id.Value = m_glassMatID;
            glass1.ParametersMap.get_Item("Material").Set(ref id);
            glass2.ParametersMap.get_Item("Material").Set(ref id);            
            m_document.EndTransaction();
        }
        ...
        { 
            m_document.BeginTransaction();
            foreach(Material material in m_document.Settings.Materials)
            {
                if (0 == material.Name.CompareTo(m_para.SashMat))
                {                   
                    m_sashMatID = material.Id.Value;
                }

                if (0 == material.Name.CompareTo(m_para.GlassMat))
                {                    
                    m_glassMatID = material.Id.Value;
                }
            }
            m_document.EndTransaction();
        }
        ...
        private void CollectTemplateInfo()
        {
            ...
            //get materials
            Materials materialSet = m_document.Settings.Materials;
