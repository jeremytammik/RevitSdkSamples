    ...
    class Intersection
    {
        ...
        public static bool IsOverlapped(GeometryObject geometryA, GeometryObject geometryB)
        {
            ...
            foreach (Face face in facesOfA)
            {
                ...
                {
                    if (face.Intersect(curve) == Autodesk.Revit.Enums.SetComparisonResult.Overlap)
                    {
                        return true;
                    }
                }
        ...
        private static void GetAllCurves(Face face, List<Curve> curves)
        {
            foreach (EdgeArray loop in face.EdgeLoops)
            {
                foreach (Edge edge in loop)
                {
                    ...
                    for(int ii = 0; ii + 1 < points.Size; ii++)
                    {
                        Line line = Command.s_appCreation.NewLineBound(points.get_Item(ii), points.get_Item(ii + 1));
                        curves.Add(line);
                    }
                }
            }
