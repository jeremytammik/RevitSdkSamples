...
namespace Revit.SDK.Samples.ObjectViewer.CS
{
    ...
    public class Graphics3DData
    {
        ...
        public Graphics3DData(List<XYZArray> curves, BoundingBoxXYZ bbox)
        {
            if (null == bbox)
            {
                return;
            ...
            foreach (XYZArray pnts in curves)
            {
        ...
        {
            XYZ size = MathUtil.SubXYZ(bbox.Max, bbox.Min);
        ...
        {
            XYZ center = MathUtil.DivideXYZ(MathUtil.AddXYZ(bbox.Max, bbox.Min), 2.0);
