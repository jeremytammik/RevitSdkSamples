...
namespace LabsCs
{
  class RvtCmd_FamilyCreateColumnVisibility : IExternalCommand
  {
    ...
    void addReferencePlanes()
    {
      ...
      //
      XYZ pBubbleEnd = new XYZ(refFront.BubbleEnd);
      XYZ pFreeEnd = new XYZ(refFront.FreeEnd);
      pBubbleEnd.Y += td;
      pFreeEnd.Y += td;
    ...
    public void addMaterialsToSolid( Extrusion pSolid )
    {
      ...
      //
      Material pMat = findElement( typeof( MaterialOther ), "Glass" ) as Material;
      if( pMat != null )
      {
        // no material with the given name.
        ElementId idMat = pMat.Id;
        pSolid.get_Parameter( "Material" ).Set( ref idMat );
      }
    }
    ...
    public void addMaterials( Extrusion pSolid )
    {
      ...
      if( pMat != null )
      {
        ...
        pFamilyMgr.Set( famParamFinish, ref idMat );
      }
    }
    ...
    public void addLineObjects()
    {
      ...
      ReferencePlane pRefPlaneH = findElement( typeof( ReferencePlane ), "Left" ) as ReferencePlane;
      SketchPlane pSketchPlaneH = _rvtDoc.FamilyCreate.NewSketchPlane( pRefPlaneH.Plane );
      ...
      foreach( GeometryObject geomObj in geomObjs )
      {
        if( geomObj is Solid ) // solid is what we are interested in.
        {
          ...
          foreach( Face pFace in faces )
          {
            ...
            if( (pPlanarFace != null) && pPlanarFace.Normal.AlmostEqual( normal ) )
            {
