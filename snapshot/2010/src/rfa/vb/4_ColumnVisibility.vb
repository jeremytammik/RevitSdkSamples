...
Public Class RvtCmd_FamilyCreateColumnVisibility
    ...
    '' ============================================
    Function TemplateCategoryEquals(ByVal targetCategory As BuiltInCategory) As Boolean
    ...
    Sub addReferencePlanes()
        ...
        ''
        Dim pBubbleEnd As New XYZ(refFront.BubbleEnd)
        Dim pFreeEnd As New XYZ(refFront.FreeEnd)
        pBubbleEnd.Y += td
        pFreeEnd.Y += td

        ''  create the new one
        ''
        Dim refPlane As ReferencePlane = _rvtDoc.FamilyCreate.NewReferencePlane(pBubbleEnd, pFreeEnd, XYZ.BasisZ, pViewPlan)
        ...
        ''  get the bubble and free ends from front ref plane and offset by td.
        ''
        pBubbleEnd = New XYZ(refLeft.BubbleEnd)
        pFreeEnd = New XYZ(refLeft.FreeEnd)
        pBubbleEnd.X += tw
        pFreeEnd.X += tw
    ...
    Sub addAlignments(ByVal pBox As Extrusion)
        ...
        _rvtDoc.FamilyCreate.NewAlignment(pViewPlan, refOffsetV.Reference, faceOffsetV.Reference)
        _rvtDoc.FamilyCreate.NewAlignment(pViewPlan, refOffsetH.Reference, faceOffsetH.Reference)
    ...
    Sub addParameters()
        ...
        ''
        Dim paramTw As FamilyParameter = _rvtDoc.FamilyManager.AddParameter("Tw", BuiltInParameterGroup.PG_GEOMETRY, ParameterType.Length, False)
        Dim paramTd As FamilyParameter = _rvtDoc.FamilyManager.AddParameter("Td", BuiltInParameterGroup.PG_GEOMETRY, ParameterType.Length, False)
    ...
    Sub addDimensions()
        ...
        Dim pLine As Line = _rvtApp.Create.NewLineBound(p0, p1)
        ...
        ''  create a dimension
        ''
        Dim pDimTw As Dimension = _rvtDoc.FamilyCreate.NewDimension(pViewPlan, pLine, pRefArray)
        ...
        ''  create a dimension
        ''
        Dim pDimTd As Dimension = _rvtDoc.FamilyCreate.NewDimension(pViewPlan, pLine, pRefArray)

        ''  add label to the dimension
        ''
        Dim paramTd As FamilyParameter = _rvtDoc.FamilyManager.Parameter("Td")
    ...
    Sub addMaterialsToSolid(ByVal pSolid As Extrusion)
        ...
        ''
        Dim pMat As Elements.Material = findElement(GetType(Elements.MaterialOther), "Glass") ' hard coded fot simplicity.
    ...
    Sub addLineObjects()
        ...
        Dim pRefPlaneH As ReferencePlane = findElement(GetType(ReferencePlane), "Left") ' need to know from the template
        Dim pSketchPlaneH As SketchPlane = _rvtDoc.FamilyCreate.NewSketchPlane(pRefPlaneH.Plane)

        ''  (4) create line objects: two symbolic curves on a plan and one model curve representing a column like a vertical stick.
        ''
        Dim geomLine1 As Geometry.Line = _rvtApp.Create.NewLine(pts(0), pts(1), True)
        Dim geomLine2 As Geometry.Line = _rvtApp.Create.NewLine(pts(1), pts(2), True)
        Dim geomLineH As Geometry.Line = _rvtApp.Create.NewLine(pts(1), ptH, True)

        Dim pLine1 As SymbolicCurve = _rvtDoc.FamilyCreate.NewSymbolicCurve(geomLine1, pSketchPlane)
        Dim pLine2 As SymbolicCurve = _rvtDoc.FamilyCreate.NewSymbolicCurve(geomLine2, pSketchPlane)

        Dim pLineH As ModelCurve = _rvtDoc.FamilyCreate.NewModelCurve(geomLineH, pSketchPlaneH) ' this is vertical line
            ...
            If TypeOf geomObj Is Geometry.Solid Then  ''  solid is what we are interested in.
                ...
                For Each pFace As Face In faces
                    ...
                    If Not (pPlanarFace Is Nothing) Then
                        ...
                        If pPlanarFace.Normal.AlmostEqual(normal) Then
