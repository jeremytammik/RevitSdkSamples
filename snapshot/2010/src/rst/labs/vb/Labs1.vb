...
Namespace RstLabs
    ...
    ''' <summary>
    ''' Lab 1-1 - List all load grouping objects, i.e. 
    ''' load natures, cases, and combinations.
    ''' </summary>
    Public Class Lab1_1_LoadGrouping
        ...
        Implements IExternalCommand.Execute
            ...
            ' Load NATURES
            Dim ldNatures As List(Of Revit.Element) = RstUtils.GetAllLoadNatures(app)
            Dim sMsg As String = "There are " & ldNatures.Count & " LoadNature objects in the model:" & vbCrLf
            ...
            ' Load CASES
            Dim ldCases As List(Of Revit.Element) = RstUtils.GetAllLoadCases(app)
            sMsg = "There are " & ldCases.Count & " LoadCase objects in the model:" & vbCrLf
            For Each ldCase As LoadCase In ldCases
                ...
                If (ldCase.Category Is Nothing) Then
                    catName = ""
                Else
            ...
            Dim ldUsages As List(Of Revit.Element) = RstUtils.GetAllLoadUsages(app)
            sMsg = "There are " & ldUsages.Count & " LoadUsage objects in the model:" & vbCrLf
            ...
            ' Load COMBINATIONS
            Dim combs As List(Of Revit.Element) = RstUtils.GetAllLoadCombinations(app)
            sMsg = "There are " & combs.Count & " LoadCombination objects in the model:" & vbCrLf
            For Each comb As LoadCombination In combs
                ...
                ' Loop all components' properties
    ...
    Public Class Lab1_2_Loads
        ...
        Implements IExternalCommand.Execute
            ...
            Dim loads As List(Of Revit.Element) = RstUtils.GetAllLoads(app)
            Dim sMsg As String = "There are " & loads.Count & " Load objects in the model:" & vbCrLf

            Dim sHost As String
            Dim load As LoadBase
            For Each load In loads
                If (load.HostElement Is Nothing) Then
                    sHost = ""
                Else
            ...
            For Each ptLd In pointLoads
                ...
                sMsg += "Id=" & ptLd.Id.Value.ToString & _
                        ": F= " & RacUtils.PointString(F) & _
                        " M= " & RacUtils.PointString(M) & _
                        " Pt= " & RacUtils.PointString(pt) & _
            ...
            For Each lnLd In lineLoads
                ...
                sMsg += "Id=" & lnLd.Id.Value.ToString & _
                        ": F1= " & RacUtils.PointString(F1) & _
                        ": F2= " & RacUtils.PointString(F2) & _
                        " Start= " & RacUtils.PointString(ptStart) & _
            ...
            For Each arLd In areaLoads
                ...
                sMsg += "Id=" & arLd.Id.Value.ToString & _
                        ": F1= " & RacUtils.PointString(F1) & s2 & vbCrLf
    ...
    Public Class Lab1_3_ModifySelectedLoads
        ...
        Public Sub ListAndModifyPointLoad(ByVal ptLd As PointLoad)
            ...
            Try
                ...
                sMsg += "  Is Reaction (via Display Name)= " & RacUtils.GetParameterValue(RacUtils.GetElemParam(ptLd, "Is Reaction")) & vbCrLf 'Int
                sMsg += "  Load Case (via Display Name)= " & RacUtils.GetParameterValue(RacUtils.GetElemParam(ptLd, "Load Case")) & vbCrLf ' Id
                ...
                sMsg += "  LOAD_IS_REACTION BuiltInParameter= " & RacUtils.GetParameterValue(ptLd.Parameter(BuiltInParameter.LOAD_IS_REACTION)) & vbCrLf 'Int
                sMsg += "  LOAD_CASE_ID BuiltInParameter= " & RacUtils.GetParameterValue(ptLd.Parameter(BuiltInParameter.LOAD_CASE_ID)) & vbCrLf ' Id

                ' The following Display one hasn't got one-to-one corresponding Built-in, but basically the same:
                sMsg += "  Orient to (via Display Name)= " & RacUtils.GetParameterValue(RacUtils.GetElemParam(ptLd, "Orient to")) & vbCrLf ' Int
                sMsg += "  LOAD_USE_LOCAL_COORDINATE_SYSTEM BuiltInParameter= " & RacUtils.GetParameterValue(ptLd.Parameter(BuiltInParameter.LOAD_USE_LOCAL_COORDINATE_SYSTEM)) & vbCrLf '
                Try ' this param was working in RS2, but not accessible in RS3! ... is there in 2008
                    sMsg += "  LOAD_USE_LOCAL_COORDINATE_SYSTEM_HOSTED BuiltInParameter= " & RacUtils.GetParameterValue(ptLd.Parameter(BuiltInParameter.LOAD_USE_LOCAL_COORDINATE_SYSTEM_HOSTED)) & vbCrLf '
                ...
                Try  ' this param was working in RS2, but not accessible in RS3!
                    sMsg += "  LOAD_IS_CREATED_BY_API BuiltInParameter= " & RacUtils.GetParameterValue(ptLd.Parameter(BuiltInParameter.LOAD_IS_CREATED_BY_API)) & vbCrLf 'Int
                Catch
                End Try

                Try
                    sMsg += "  ELEM_TYPE_PARAM BuiltInParameter= " & RacUtils.GetParameterValue(ptLd.Parameter(BuiltInParameter.ELEM_TYPE_PARAM)) & vbCrLf
                Catch
                End Try
        ...
        Public Sub ListAndModifyLineLoad(ByVal lnLd As LineLoad, ByVal app As Revit.Application)
            ...
            Try
                ...
                sMsg += "  Load Case (via Display Name)= " & RacUtils.GetParameterValue(RacUtils.GetElemParam(lnLd, "Load Case")) & vbCrLf ' Id
                sMsg += "  LOAD_CASE_ID BuiltInParameter= " & RacUtils.GetParameterValue(lnLd.Parameter(BuiltInParameter.LOAD_CASE_ID)) & vbCrLf ' Id
                ...
                sMsg += "  Orient to (via Display Name)= " & RacUtils.GetParameterValue(RacUtils.GetElemParam(lnLd, "Orient to")) & vbCrLf ' Int
                sMsg += "  LOAD_USE_LOCAL_COORDINATE_SYSTEM BuiltInParameter= " & RacUtils.GetParameterValue(lnLd.Parameter(BuiltInParameter.LOAD_USE_LOCAL_COORDINATE_SYSTEM)) & vbCrLf '
                Try
                    sMsg += "  LOAD_USE_LOCAL_COORDINATE_SYSTEM_HOSTED BuiltInParameter= " & RacUtils.GetParameterValue(lnLd.Parameter(BuiltInParameter.LOAD_USE_LOCAL_COORDINATE_SYSTEM_HOSTED)) & vbCrLf '
                Catch
                End Try
                ...
                ' If created by API
                Try  ' this param was working in RS2, but not accessible in RS3!
                    sMsg += "  LOAD_IS_CREATED_BY_API BuiltInParameter= " & RacUtils.GetParameterValue(lnLd.Parameter(BuiltInParameter.LOAD_IS_CREATED_BY_API)) & vbCrLf 'Int
                Catch
                End Try
        ...
        Public Sub ListAndModifyAreaLoad(ByVal arLd As AreaLoad, ByVal app As Revit.Application)
            ...
            Try
                ...
                ' Again, show how to access the same param via both display name and BUILT_IN_PARAMS
                sMsg += "  Load Case (via Display Name)= " & RacUtils.GetParameterValue(RacUtils.GetElemParam(arLd, "Load Case")) & vbCrLf ' Id
                sMsg += "  LOAD_CASE_ID BuiltInParameter= " & RacUtils.GetParameterValue(arLd.Parameter(BuiltInParameter.LOAD_CASE_ID)) & vbCrLf ' Id
                    ...
                    For iCurve = 0 To numCurves - 1
                        ...
                        If TypeOf crv Is Geometry.Line Then      'LINE
                            ...
                            sMsg += "      Curve " & iCurve + 1 & " is a LINE:" & _
                             RacUtils.PointString(ptS) & " ; " & _
                             RacUtils.PointString(ptE) & vbCrLf
                        ...
                        ElseIf TypeOf crv Is Geometry.Arc Then      'ARC
                            ...
                            sMsg += "      Curve " & iCurve + 1 & " is an ARC:" & _
                             RacUtils.PointString(ptS) & " ; " & _
                             RacUtils.PointString(ptE) & " ; R=" & r & vbCrLf
                ...
                For iRefPt = 0 To numRefPts - 1
                    ...
                    sMsg += "    RefPt " & iRefPt + 1 & " = " & RacUtils.PointString(pt) & vbCrLf
                Next
    ...
    ''' </summary>
    Public Class Lab1_4_LoadSymbols
        Implements IExternalCommand
        ...
        Implements IExternalCommand.Execute
            ...
            Dim symbs As List(Of Revit.Element) = RstUtils.GetAllLoadSymbols(app)
            Dim sMsg As String = "There are " & symbs.Count & " Load Symbol objects in the model:" & vbCrLf
            ...
            Dim ptLdSymbs As List(Of Revit.Element) = RstUtils.GetPointLoadSymbols(app)
            ...
            For Each ls In ptLdSymbs
                ...
                Dim scaleF As String = RacUtils.GetParameterValue(ls.Parameter(BuiltInParameter.LOAD_ATTR_FORCE_SCALE_FACTOR))
