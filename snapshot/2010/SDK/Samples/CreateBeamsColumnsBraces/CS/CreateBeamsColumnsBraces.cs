    ...
    public class Command : IExternalCommand
    {
      ...
      public IExternalCommand.Result Execute(Autodesk.Revit.ExternalCommandData revit, ref string message, ElementSet elements)
      {
        ...
        private void PlaceColumn(UV point2D, FamilySymbol columnType, Level baseLevel, Level topLevel)
        {
            ...
            FamilyInstance column = m_revit.ActiveDocument.Create.NewFamilyInstance( point, columnType, topLevel, structuralType);
        ...
        private void PlaceBeam(UV point2D1, UV point2D2, Level baseLevel, Level topLevel, FamilySymbol beamType)
        {
            ...
            FamilyInstance beam = m_revit.ActiveDocument.Create.NewFamilyInstance(startPoint, beamType, topLevel, structuralType);

            LocationCurve beamCurve = beam.Location as LocationCurve;
            if (null != beamCurve)
            {
                Line line = m_revit.Create.NewLineBound(startPoint, endPoint);
        ...
        private void PlaceBrace(UV point2D1, UV point2D2, Level baseLevel, Level topLevel, FamilySymbol braceType, bool isXDirection)
        {
            ...
            FamilyInstance firstBrace = m_revit.ActiveDocument.Create.NewFamilyInstance(startPoint, braceType, structuralType);
            LocationCurve braceCurve1 = firstBrace.Location as LocationCurve;
            if (null != braceCurve1)
            {
                Line line = m_revit.Create.NewLineBound(startPoint, middlePoint);
            ...
            FamilyInstance secondBrace = m_revit.ActiveDocument.Create.NewFamilyInstance(endPoint, braceType, baseLevel, structuralType);
            LocationCurve braceCurve2  = secondBrace.Location as LocationCurve;
            if (null != braceCurve2)
            {
                Line line = m_revit.Create.NewLineBound(endPoint, middlePoint);
