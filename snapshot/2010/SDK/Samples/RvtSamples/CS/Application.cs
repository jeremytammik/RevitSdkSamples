...
namespace RvtSamples
{
    ...
    public class Application : IExternalApplication
    {
        ...
        #region Member Data
        /// <summary>
        /// Full path of input file after resolution
        /// </summary>
        ...
        /// </summary>
        private ControlledApplication m_application;
        /// <summary>
        ...
        public IExternalApplication.Result OnStartup(ControlledApplication application)
        {
            ...
            try
            {
                ...
                foreach(string category in Enum.GetNames(typeof(DefaultPulldownMenus)))
                {
                    ...
                    if ((i + 1) % 3 == 0)
                    {
                        List<RibbonItem> addedButtons = m_panelRvtSamples.AddStackedButtons(pdData[0], pdData[1], pdData[2]);
        ...
        {
            MessageBox.Show(msg, "RvtSamples", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        ...
        void AddSample(string[] lines, int n, ref int i)
        {
            ...
            {
              Debug.Print( "RvtSamples: Assembly '{0}' specified in line {1} of {2} not found", 
            ...
            if( testClassName )
            {
              try
              {
                ...
                if( null == a )
                {
                  Debug.Print( "RvtSamples: Unable to load assembly '{0}' specified in line {1} of {2}", 
                    assembly, i, m_fileNameStem );
                }
                else
                {
                  ...
                  if( null == t )
                  {
                    Debug.Print( "RvtSamples: External command class {0} in assembly '{1}' specified in line {2} of {3} not found", 
                  ...
                  else
                  {
                    ...
                    if( null == m )
                    {
                      Debug.Print( "RvtSamples: External command class {0} in assembly '{1}' specified in line {2} of {3} does not define an Execute method", 
                        className, assembly, i, m_fileNameStem );
                    }
                  }
                }
              ...
              {
                Debug.Print( "RvtSamples: Exception '{0}' testing assembly '{1}' specified in line {2} of {3}", 
        ...
        private void AddSampleToPulldownMenu(
          PulldownButton pullDownButton,
          SampleItem item )
        {
          PushButton pushButton = pullDownButton.AddItem( 
            item.DisplayName, item.Assembly, item.ClassName );

          if( !string.IsNullOrEmpty( item.LargeImage ) )
          {
            BitmapImage largeImageSource = new BitmapImage( 
              new Uri( item.LargeImage, UriKind.Absolute ) );
          ...
          if( !string.IsNullOrEmpty( item.Image ) )
          {
            BitmapImage imageSource = new BitmapImage( 
              new Uri( item.Image, UriKind.Absolute ) );
        ...
        private void AddCustomizedPulldownMenus()
        {
            ...
            while (iCount >= 3)
            {
                ...
                List<RibbonItem> buttons = m_panelRvtSamples.AddStackedButtons(data1, data2, data3);
            ...
            if (iCount == 2)
            {
                ...
                name = m_customizedMenus.Keys[i++];
                PulldownButtonData data2 = new PulldownButtonData(name, name);
                List<RibbonItem> buttons = m_panelRvtSamples.AddStackedButtons(data1, data2);
                AddSamplesToStackedButtons(buttons);
            }
            else if (iCount == 1)
            {
                string name = m_customizedMenus.Keys[i];
                PulldownButton button = m_panelRvtSamples.AddPulldownButton(name, name);
        ...
        private void AddSamplesToStackedButtons(List<RibbonItem> buttons)
        {
