...
namespace Revit.SDK.Samples.DoorSwing.CS
{
   ...
   public class DoorGeometry
   {
      ...
      List<XYZArray> m_curve3Ds = new List<XYZArray>();
      ...
      public DoorGeometry(Autodesk.Revit.Element door)
      {
         m_options                              = new Options();
         m_options.View                         = GetPlanform2DView(door);
         m_options.ComputeReferences            = false;
         Autodesk.Revit.Geometry.Element geoEle = door.get_Geometry(m_options);
      ...
      public void DrawGraphics(System.Drawing.Graphics graphics, System.Drawing.Pen drawPen)
      {
         ...
         {
            XYZArray points = m_curve3Ds[i];

            for (int j = 0; j < (points.Size - 1); j++)
            {
               // ignore XYZ.Z value, drawn as top view.
               System.Drawing.PointF startPoint = new System.Drawing.PointF((float)points.get_Item(j).X, (float)points.get_Item(j).Y);
               System.Drawing.PointF endPoint   = new System.Drawing.PointF((float)points.get_Item(j + 1).X, (float)points.get_Item(j + 1).Y);
      ...
      static private ViewPlan GetPlanform2DView(Autodesk.Revit.Element door)
      {
         ElementIterator iter = door.Document.Elements;
         while (iter.MoveNext())
         {
            ViewPlan viewPlan = iter.Current as ViewPlan;
            ...
            if (viewPlan.GenLevel.Id.Value == door.Level.Id.Value)
            {
               return viewPlan;
            }
         }
      ...
      private void AddGeometryElement(Autodesk.Revit.Geometry.Element geoEle)
      {
         ...
         foreach (GeometryObject geoObj in geoObjArray)
         {
            ...
            else if (geoObj is Autodesk.Revit.Geometry.Element)
            {
               AddElement(geoObj);
            }
            ...
            else if (geoObj is Autodesk.Revit.Geometry.Instance)
            {
               AddInstance(geoObj);
            }
      ...
      {
         Autodesk.Revit.Geometry.Element geoEle = obj as Autodesk.Revit.Geometry.Element;
         AddGeometryElement(geoEle);
      }
      ...
      {
         Autodesk.Revit.Geometry.Instance instance  = obj as Autodesk.Revit.Geometry.Instance;
         Autodesk.Revit.Geometry.Element geoElement = instance.SymbolGeometry;
      ...
      private void AddMesh(GeometryObject obj)
      {
         ...
         for (int i = 0; i < mesh.NumTriangles; i++)
         {
            ...
            for (int j = 0; j < 3; j++)
            {
               ...
               XYZ point = trigangle.get_Vertex(j);
