    ...
    public class Command : IExternalCommand
    {
        ...
        private void CreateExtrusions()
        {
            ...
            for (int i = 0; i <= 2; ++i)
            {
                ...
                if (i == 0)
                {
                    // Main body box, change it to have a Glass material.

                    Parameter oExtrusionMaterialParam = extrusions[i].get_Parameter(BuiltInParameter.MATERIAL_ID_PARAM);

                    Material oGlassMaterial = null;
                    ElementId oTempElementID;

                    // Do what the GUI does, and pick from the list of materials available to this 
                    // document.
                    foreach (Material oMaterial in m_document.Settings.Materials)
                    {
                        if (oMaterial.Name == "Glass")
                        {
                            // We've found the correct material, assign it to this parameter as the user would
                            // pick it from the list of available materials.
                            oGlassMaterial = oMaterial;
                            
                            oTempElementID = oGlassMaterial.Id;  // Need this temp variable or the "ref" below won't work
                            
                            // This does work.
                            oExtrusionMaterialParam.Set(ref oTempElementID);

                            break;
                        }
                    }

                    // THIS METHOD DOES WORK, IF THE FAMILY PARAMETER IS SET UP CORRECTLY
                    // Do what the GUI does, and link to a family parameter that has the value set.
                    FamilyParameter oFamParam = m_document.FamilyManager.AddParameter("My Material", 
                                                                                      BuiltInParameterGroup.PG_MATERIALS, 
                                                                                      ParameterType.Material, 
                                                                                      false);

                    // m_document.FamilyManager.AssociateElementParameterToFamilyParameter(oExtrusionMaterialParam, oFamParam);

                    
                    // BUT HOW DO WE SET THE FAMILY PARAMETER'S VALUE FOR THIS MATERIAL WITH CODE?
                    oTempElementID = oGlassMaterial.Id;
                    m_document.FamilyManager.Set(oFamParam, ref oTempElementID);

                    m_document.FamilyManager.AssociateElementParameterToFamilyParameter(oExtrusionMaterialParam, oFamParam);

                }  // End of the If check to see if this is the main body's extrusion.
        ...
        private void CreateConnectors()
        {
            ...
            DuctConnector connSupplyAir = f.NewDuctConnector(m_planarFaces[5].Reference,
