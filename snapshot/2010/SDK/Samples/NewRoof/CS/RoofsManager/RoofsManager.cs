...
namespace Revit.SDK.Samples.NewRoof.RoofsManager.CS
{
    ...
    public class RoofsManager
    {
        ...
        Autodesk.Revit.Geometry.CurveArray m_footPrint;

        // To store the profile lines.
        Autodesk.Revit.Geometry.CurveArray m_profile;
        ...
        public RoofsManager(ExternalCommandData commandData)
        {
            ...
            m_selection = commandData.Application.ActiveDocument.Selection;
        ...
        private void Initialize()
        {
            ...
            ElementIterator iter = m_commandData.Application.ActiveDocument.get_Elements(levelsFilter);
            iter.Reset();
            while (iter.MoveNext())
            {
            ...
            foreach(RoofType roofType in m_commandData.Application.ActiveDocument.RoofTypes)
            {
                m_roofTypes.Add(roofType);
            }

            // FootPrint Roofs
            TypeFilter footPrintRoofFilter = m_commandData.Application.Create.Filter.NewTypeFilter(typeof(FootPrintRoof));
            m_footPrintRoofs = new ElementSet();
            iter = m_commandData.Application.ActiveDocument.get_Elements(footPrintRoofFilter);
            iter.Reset();
            while (iter.MoveNext())
            {
            ...
            // Extrusion Roofs
            TypeFilter extrusionRoofFilter = m_commandData.Application.Create.Filter.NewTypeFilter(typeof(ExtrusionRoof));
            m_extrusionRoofs = new ElementSet();
            iter = m_commandData.Application.ActiveDocument.get_Elements(extrusionRoofFilter);
            iter.Reset();
            while (iter.MoveNext())
            {
            ...
            // Reference Planes
            TypeFilter refPlaneFilter = m_commandData.Application.Create.Filter.NewTypeFilter(typeof(ReferencePlane));
            iter = m_commandData.Application.ActiveDocument.get_Elements(refPlaneFilter);
            iter.Reset();
            while (iter.MoveNext())
            {
                ReferencePlane plane = iter.Current as ReferencePlane;
                // just use the vertical plane
                if (Math.Abs(plane.Normal.Dot(XYZ.BasisZ)) < 1.0e-09)
                {
                    if (plane.Name == "Reference Plane")
                    {
                        plane.Name = "Reference Plane" + "(" + plane.Id.Value.ToString() + ")"; 
                    }
        ...
        public CurveArray SelectFootPrint()
        {
            ...
            while (true)
            {
                ...
                m_selection.WindowSelect();
                
                if (m_selection.Elements.Size != 0)
                {
                    foreach (Autodesk.Revit.Element element in m_selection.Elements)
                    {
        ...
        {
            return m_commandData.Application.ActiveDocument.BeginTransaction();
        }
        ...
        {
            return m_commandData.Application.ActiveDocument.EndTransaction();
        }
