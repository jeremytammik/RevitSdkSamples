...
namespace Revit.SDK.Samples.Loads.CS
{
    ...
    public class LoadCombinationDeal
    {
        ...
        Autodesk.Revit.Document m_document; // Store the reference of document
        ...
        public LoadCombinationDeal(Loads dataBuffer)
        {
            ...
            m_document = m_revit.ActiveDocument;
        }
        ...
        {
            // Find out all  Load Combination and Usage in the existing document.
            ElementIterator i = m_document.Elements;
            i.Reset();
            bool canMove = i.MoveNext();
            while (canMove)
            {
                // Add Load Combination information
                LoadCombination combination = i.Current as LoadCombination;
                if (null != combination)
                {
                ...
                LoadUsage usage = i.Current as LoadUsage;
                if (null != usage)
                {
        ...
        public Boolean NewLoadCombination(String name, int typeId, int stateId)
        {
            ...
            // If create combination succeeful, reset the usage check state and clear the formula
            foreach (UsageMap usageMap in m_dataBuffer.UsageMap)
            {
                usageMap.Set = false;
            }
        ...
        public Boolean DeleteCombination(int index)
        {
            ...
            ElementIterator iterator = m_document.Elements;
            iterator.Reset();
            while (iterator.MoveNext())
            {
                combination = iterator.Current as LoadCombination;
                if (null == combination)
                {
                    continue;
                }
                ...
                {
                    // Begin to delete the combinnation
                    try
                    {
