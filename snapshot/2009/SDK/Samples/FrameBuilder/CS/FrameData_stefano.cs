...
namespace Revit.SDK.Samples.FrameBuilder.CS
{
	...
	public class FrameData
	{
		...
		{
            //// OLD VERSION -> LANGUAGE DEPENDENT
            ...
            Categories categories = m_commandData.Application.ActiveDocument.Settings.Categories;
            BuiltInCategory oColumns = BuiltInCategory.OST_StructuralColumns;
            BuiltInCategory oFraming = BuiltInCategory.OST_StructuralFraming;
            ElementId idColumn = categories.get_Item(oColumns).Id;
            ElementId idFraming = categories.get_Item(oFraming).Id;
            Application app = m_commandData.Application;
            Document doc = m_commandData.Application.ActiveDocument;
            
            List<Autodesk.Revit.Element> a = new List<Autodesk.Revit.Element>();

            Filter f = app.Create.Filter.NewTypeFilter(typeof(Level));
            ...
            a.Clear();
            Filter filterSymbols = app.Create.Filter.NewTypeFilter(typeof(FamilySymbol));
            Filter filterCols    = app.Create.Filter.NewCategoryFilter(oColumns);
            Filter filterFram    = app.Create.Filter.NewCategoryFilter(oFraming);
            Filter filterStruct  = app.Create.Filter.NewLogicOrFilter(filterCols, filterFram);
            Filter filter        = app.Create.Filter.NewLogicAndFilter(filterSymbols, filterStruct);
