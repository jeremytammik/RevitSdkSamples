...
namespace Revit.SDK.Samples.DoorSwing.CS
{
   ...
   public class DoorFamily
   {
      ...
      public DoorFamily(Family doorFamily, Application app)
      {
         m_app         = app;
         m_family      = doorFamily;
      ...
      public void DeleteTempDoorInstance()
      {
         ...
         Autodesk.Revit.Element tempWall = m_oneInstance.Host;
      ...
      {
         Autodesk.Revit.Document doc                = m_app.ActiveDocument;
         Autodesk.Revit.Creation.Document creDoc    = doc.Create;
         Autodesk.Revit.Creation.Application creApp = m_app.Create; 

         // get one level. A project has at least one level.
         TypeFilter levelFilter   = creApp.Filter.NewTypeFilter(typeof(Level), true);
         ElementIterator elemIter = doc.get_Elements(levelFilter);
         elemIter.MoveNext();
         Level level              = elemIter.Current as Level;
         
         // create one wall as door's host
         Line wallCurve = creApp.NewLineBound(new XYZ(0,0,0), new XYZ(100, 0, 0));
         ...
         // create the door
         return creDoc.NewFamilyInstance(new XYZ(0, 0, 0), doorSymbol, host,level, 
