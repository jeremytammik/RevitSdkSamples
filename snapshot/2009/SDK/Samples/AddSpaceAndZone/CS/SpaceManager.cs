...
namespace Revit.SDK.Samples.AddSpaceAndZone.CS
{
    ...
    class SpaceManager
    {
        ...
        {
            return m_spaceDictionary[level.Id.Value];
        }
        ...
        public void CreateSpaces(Level level, Phase phase)
        {
            try
            {               
                ElementSet elements = m_commandData.Application.ActiveDocument.Create.NewSpaces(level, phase, this.m_commandData.Application.ActiveDocument.ActiveView);
                foreach (Element elem in elements)
                {
                    Space space = elem as Space;
                    if (space != null)
                    {
                        m_spaceDictionary[level.Id.Value].Add(space);
                    }
                }
                if (elements == null || elements.Size == 0)
                {
                    System.Windows.Forms.MessageBox.Show("There is no enclosed loop in " + level.Name);
                }
