    ...
    public class CreateWallsUnderBeams : IExternalCommand
    {
        ...
        Boolean BeginCreate(Autodesk.Revit.Document project)
        {
            ...
            for (int i = 0; i < m_beamCollection.Count; i++)
            {
                ...
                Double offset = model.Curve.get_EndPoint(0).Z - m_level.Elevation;
        ...
        Boolean CheckBeamHorizontal()
        {
            for (int i = 0; i < m_beamCollection.Count; i++)
            {
                ...
                else if ((PRECISION <= model.Curve.get_EndPoint(0).Z - model.Curve.get_EndPoint(1).Z)
                    || (-PRECISION >= model.Curve.get_EndPoint(0).Z - model.Curve.get_EndPoint(1).Z))
                {
