...
{
   //using structualEnum as Autodesk.Revit.Structural.Enums
   using StructualEnum = Autodesk.Revit.Structural.Enums;

   /// <summary>
   /// The class derived from FramReinMaker showes how to create the rebars for a beam
   /// </summary>
   public class BeamFramReinMaker : FramReinMaker
   {
         ...
         : base(commandData, hostObject)
      {
         ...
         Options geoOptions = commandData.Application.Create.NewGeometryOptions();
      ...
      public bool FillBottomBars()
      {
         ...
         Rebar rebar = PlaceRebars(m_bottomType, null, null, geomInfo,
                                 HookOrient.Orient0, HookOrient.Orient0);
         return (null != rebar);
      }
      ...
      public Rebar FillTransverseBar(TransverseRebarLocation location)
      {
         ...
         HookOrient startHook = HookOrient.Orient1;
         HookOrient endHook = HookOrient.Orient3;
         if (!GeomUtil.IsInRightDir(geomInfo.Normal))
          {
              startHook = HookOrient.Orient3;
              endHook = HookOrient.Orient1;
          }
      ...
      private HookOrient GetTopHookOrient(RebarGeometry geomInfo, TopRebarLocation location)
      {
         ...
         XYZ hookVec = m_geometry.GetDownDirection();
         XYZ normal = geomInfo.Normal;
         Line rebarLine = geomInfo.Curves.get_Item(0) as Line;

         // get the top start hook orient
         if (TopRebarLocation.Start == location)
         {
            XYZ curveVec = GeomUtil.SubXYZ(rebarLine.get_EndPoint(1), rebarLine.get_EndPoint(0));
            return GeomUtil.GetHookOrient(curveVec, normal, hookVec);
         }
         else    // get the top end hook orient
         {
            XYZ curveVec = GeomUtil.SubXYZ(rebarLine.get_EndPoint(0), rebarLine.get_EndPoint(1));
            return GeomUtil.GetHookOrient(curveVec, normal, hookVec);
         }
      }
      ...
      private Rebar FillTopBar(TopRebarLocation location)
      {
         ...
         HookOrient startOrient = HookOrient.Orient0;// the start hook orient
         HookOrient endOrient = HookOrient.Orient0;  // the end hook orient
