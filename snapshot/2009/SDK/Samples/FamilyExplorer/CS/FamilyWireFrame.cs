    ...
    public class FamilyWireFrame
    {
        ...
        public void AutoFit(double width, double height)
        {
            ...
            if (!this.IsEmpty)// rotate this family to appropriate orientation
            {
                ...
                XYZ max = m_bBox.Max;
                XYZ min = m_bBox.Min;
                Transform rotate = Transform.get_Rotation(origin, axis, angle);
                m_bBox.Max = rotate.OfPoint(max);
                m_bBox.Min = rotate.OfPoint(min);

                // rotate the orientation
                m_upDirection = rotate.OfVector(m_upDirection).Normalized;
                m_rightDirection = rotate.OfVector(m_rightDirection).Normalized;
                m_frontDirection = rotate.OfVector(m_frontDirection).Normalized;
                ...
                trf = Transform.get_Translation(move);
                max = m_bBox.Max;
                m_bBox.Max = trf.OfPoint(max);
                min = m_bBox.Min;
                m_bBox.Min = trf.OfPoint(min);
                m_origin = trf.OfPoint(m_origin);

                // translate the orientation
                m_upDirection = trf.OfVector(m_upDirection).Normalized;
                m_rightDirection = trf.OfVector(m_rightDirection).Normalized;
                m_frontDirection = trf.OfVector(m_frontDirection).Normalized;
            }
        }
        ...
        public int Rotate(int rotateAxisMode, double angle)
        {
            int num = 0;
            XYZ origin = m_origin;
        ...
        public int Translate(XYZ move)
        {
            ...
            // translate the bounding box
            Transform trf = Transform.get_Translation(move);            
            XYZ max = m_bBox.Max;
            m_bBox.Max = trf.OfPoint(max);
            XYZ min = m_bBox.Min;
            m_bBox.Min = trf.OfPoint(min);
