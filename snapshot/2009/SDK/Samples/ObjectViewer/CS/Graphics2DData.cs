...
namespace Revit.SDK.Samples.ObjectViewer.CS
{
    ...
    public class Graphics2DData
    {
        ...
        public Graphics2DData(List<UVArray> curves)
        {
            // initialize m_curves and m_bbox
            bool isFirst = true;    // is the first UVArray instance in curves
            foreach (UVArray uvs in curves)
            {
                PointF[] pnts = new PointF[uvs.Size];
                m_curves.Add(pnts);
                for (int i = 0; i < uvs.Size; i++)
                {
                    UV uv = uvs.get_Item(i);
