    ...
    class Sketch
    {
        ...
        private static void DrawLine(Pen pen, Graphics graphic, CurveArray curveArray)
        {
            foreach (Curve curve in curveArray)
            {
                ...
                if (null != line)
                {
                    PointF startPoint = new PointF((float)line.get_EndPoint(0).X, (float)line.get_EndPoint(0).Y);
                    PointF endPoint = new PointF((float)line.get_EndPoint(1).X, (float)line.get_EndPoint(1).Y);
                    graphic.DrawLine(pen, startPoint, endPoint);
                    continue;
                }
                XYZArray xyzArray = curve.Tessellate();    // Draw lines which form one arc.
                for (int i = 0; i < (xyzArray.Size - 1); i++)
                {
                    PointF startPoint = new PointF((float)xyzArray.get_Item(i).X, (float)xyzArray.get_Item(i).Y);
                    PointF endPoint = new PointF((float)xyzArray.get_Item(i + 1).X, (float)xyzArray.get_Item(i + 1).Y);
                    graphic.DrawLine(pen, startPoint, endPoint);
                }
            }
        }
