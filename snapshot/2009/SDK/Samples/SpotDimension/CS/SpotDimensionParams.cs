...
namespace Revit.SDK.Samples.SpotDimension.CS
{
    ...
    public class SpotDimensionParams
    {
        ...
        /// </summary>
        /// <param name="spotDimension">the SpotDimension need to be dealed with</param>
        /// <returns>a DataTable store Parameter information</returns>
        public DataTable GetParameterTable(Autodesk.Revit.Elements.SpotDimension spotDimension)
        {
            try
            {
                ...
                //get DimensionType
                Autodesk.Revit.Symbols.DimensionType dimensionType = spotDimension.DimensionType;
                ...
                ElementId elementId = temporaryParam.AsElementId();
                //if not found that element, add string "None" to DataTable
                if ("-1" == elementId.Value.ToString())
                {
                    temporaryValue = "None";
                }
                else
                {
                    temporaryValue = m_document.get_Element(ref elementId).Name;
                }
