...
namespace Revit.SDK.Samples.ImportExport.CS
{
    ...
    public class MainData
    {
        ...
        public MainData(ExternalCommandData commandData)
        {
            ...
            if (commandData.Application.ActiveDocument.ActiveView.ViewType == Autodesk.Revit.Enums.ViewType.ThreeD)
            {
                m_is3DView = true;
            }
            else
        ...
        public DialogResult Export(string selectedFormat)
        {
            ...
            try
            {
                switch (format)
                {
                    ...
                    case ExportFormat.DWFx2D:
                        Export2DDWFData export2DDWFData = new Export2DDWFData(m_commandData, format);
                        using (ExportWithViewsForm exportForm = new ExportWithViewsForm(export2DDWFData))
                        {
                            dialogResult = exportForm.ShowDialog();
                        }
                        break;
                    ...
                    case ExportFormat.DWF3D:
                        Export3DDWFData export3DDWFData = new Export3DDWFData(m_commandData, format);
                        dialogResult = Export(export3DDWFData);
                        break;
            ...
            {
                String errorMessage = "Failed to export " + format + " format";
        ...
        private static DialogResult Export(ExportData data)
        {
            ...
            if (result != DialogResult.Cancel)
            {
                ...
                if (!data.Export())
                {
                    MessageBox.Show("This project cannot be exported to " + data.ExportFileName + 
