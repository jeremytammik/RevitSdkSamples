  ...
  public class BuiltInParamsChecker_using_MessageBox : IExternalCommand
  {
      ...
      ElementSet elements )
    {
      ...
      foreach( BuiltInParameter a in Enum.GetValues( typeof( BuiltInParameter ) ) )
      {
        try
        {
          Parameter param = elem.get_Parameter( a );
          if( null != param ) // this check is faster than throwing an exception for each invalid param
          {
            e = a.ToString();
            n = param.Definition.Name;
            r = param.IsReadOnly ? " read-only" : string.Empty;
            v = " = '" + param.AsValueString() + "'.";
            switch( param.StorageType )
            {
              case StorageType.Double:
                v = " =" + r + " Double: " + param.AsDouble().ToString() + v;
                _dictEnum[e] = "'" + n + "'" + v;
                _dictName[n] = e + v;
                break;
              case StorageType.Integer:
                v = " =" + r + " Integer: " + param.AsInteger().ToString() + v;
                _dictEnum[e] = "'" + n + "'" + v;
                _dictName[n] = e + v;
                break;
              case StorageType.String:
                v = " =" + r + " String: '" + param.AsString() + "'" + v;
                _dictEnum[e] = "'" + n + "'" + v;
                _dictName[n] = e + v;
                break;
              case StorageType.ElementId:
                v = " =" + r + " Id: " + param.AsElementId().Value.ToString() + v;
                _dictEnum[e] = "'" + n + "'" + v;
                _dictName[n] = e + v;
                break;
      ...
      SortableBindingList<ParameterData> data = new SortableBindingList<ParameterData>();
      {
        ...
        foreach( BuiltInParameter a in bips )
        {
          ...
          if( null != param )
          {
            ...
            string valueString = LabUtils.GetParameterValueString2( param, doc );
            data.Add( new ParameterData( a, param, valueString ) );
          }
        }
      }
