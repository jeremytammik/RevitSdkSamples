...
namespace Revit.SDK.Samples.StairsAutomation.CS
{
	...
	public class LandingComponentUtils
	{
		...
		{
			double offset1 = elevation - curve.get_EndPoint(0).Z;
            Curve projectedCurve = curve.get_Transformed(Transform.get_Translation(new XYZ(0, 0, offset1)));
		...
		public static Line FindLongestEndpointConnection(Line line1, Line line2)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                for (int j = 0; j < 2; j++)
                {
                    double distance = line1.get_EndPoint(j).DistanceTo(line2.get_EndPoint(i));
                    if (distance > maxDistance)
                    {
                        maxDistance = distance;
                        endPoint1 = line1.get_EndPoint(j);
                        endPoint2 = line2.get_EndPoint(i);
                    }
                }
            }

            return Line.get_Bound(endPoint1, endPoint2);
        }
