...
namespace Revit.SDK.Samples.MultiplanarRebar.CS
{
    ...
    class Trapezoid
    {
        ...
        {
            XYZ topDir = (Top.get_EndPoint(1) - Top.get_EndPoint(0)).Normalize();
            XYZ verticalDir = (Vertical.get_EndPoint(0) - Vertical.get_EndPoint(1)).Normalize();
            XYZ normal = topDir.CrossProduct(verticalDir);

            SketchPlane sketchplane = revitDoc.Create.NewSketchPlane(new Plane(normal, Vertical.get_EndPoint(0)));
        ...
        public void OffsetTop(double offset)
        {
            ...
            XYZ verticalFinal = Vertical.get_EndPoint(0) + verticalDelta;

            double verticalLengthNew = Vertical.Length + offset;
            double slantedLengthNew = verticalLengthNew * Slanted.Length / Vertical.Length;

            XYZ slantedDir = (Slanted.get_EndPoint(1) - Slanted.get_EndPoint(0)).Normalize();
            XYZ slantedFinal = Slanted.get_EndPoint(0) + slantedDir * slantedLengthNew;

            Vertical = Line.get_Bound(verticalFinal, Vertical.get_EndPoint(1));
            Top = Line.get_Bound(slantedFinal, verticalFinal);
            Slanted = Line.get_Bound(Slanted.get_EndPoint(0), slantedFinal);
        }
        ...
        public void OffsetLeft(double offset)
        {
            ...
            XYZ topFinal = Top.get_EndPoint(1) + topDelta;
            XYZ bottomFinal = Bottom.get_EndPoint(0) + topDelta;

            Vertical = Line.get_Bound(topFinal, bottomFinal);
            Bottom = Line.get_Bound(bottomFinal, Bottom.get_EndPoint(1));
            Top = Line.get_Bound(Top.get_EndPoint(0), topFinal);
        }
        ...
        {
            XYZ verticalDir = (Vertical.get_EndPoint(1) - Vertical.get_EndPoint(0)).Normalize();
            XYZ verticalDelta = verticalDir * offset;
            XYZ verticalFinal = Vertical.get_EndPoint(1) + verticalDelta;

            double verticalLengthNew = Vertical.Length + offset;

            double slantedLengthNew = verticalLengthNew * Slanted.Length / Vertical.Length;

            XYZ slantedDir = (Slanted.get_EndPoint(0) - Slanted.get_EndPoint(1)).Normalize();
            XYZ slantedFinal = Slanted.get_EndPoint(1) + slantedDir * slantedLengthNew;

            Vertical = Line.get_Bound(Vertical.get_EndPoint(0), verticalFinal);
            Bottom = Line.get_Bound(verticalFinal, slantedFinal);
            Slanted = Line.get_Bound(slantedFinal, Slanted.get_EndPoint(1));
        }
        ...
        {
            XYZ bottomDir = (Bottom.get_EndPoint(1) - Bottom.get_EndPoint(0)).Normalize();
            XYZ bottomDelta = bottomDir * (offset * Slanted.Length / Vertical.Length);

            XYZ topFinal = Top.get_EndPoint(0) + bottomDelta;
            XYZ bottomFinal = Bottom.get_EndPoint(1) + bottomDelta;

            Top = Line.get_Bound(topFinal, Top.get_EndPoint(1));
            Bottom = Line.get_Bound(Bottom.get_EndPoint(0), bottomFinal);
            Slanted = Line.get_Bound(bottomFinal, topFinal);
        }
        ...
        {
            origin = Vertical.get_EndPoint(1);
            vX = Bottom.get_EndPoint(1) - Bottom.get_EndPoint(0);
            vY = Vertical.get_EndPoint(0) - Vertical.get_EndPoint(1);
        }
    }
