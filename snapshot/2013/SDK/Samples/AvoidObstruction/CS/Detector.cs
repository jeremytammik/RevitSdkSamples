...
namespace Revit.SDK.Samples.AvoidObstruction.CS
{
    ...
    class Detector
    {
        ...
        public List<ReferenceWithContext> Obstructions(Autodesk.Revit.DB.XYZ  origin, Autodesk.Revit.DB.XYZ  dir)
        {
            ...
            IList<ReferenceWithContext> obstructionsOnUnboundLine = m_rvtDoc.FindReferencesWithContextByDirection(origin, dir, m_view3d);
            foreach (ReferenceWithContext gRef in obstructionsOnUnboundLine)
            {
                if (gRef.GetReference().ElementReferenceType == ElementReferenceType.REFERENCE_TYPE_SURFACE)
                {
