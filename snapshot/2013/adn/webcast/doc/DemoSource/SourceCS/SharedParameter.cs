...
public class PerDocParameter : IExternalCommand
{
    ...
    public const string kParamNameVisible 
                                   = "Visible per-doc Integer";

    public const string kParamNameInvisible 
                               = "Invisible per-doc Integer";
                            ...
                            ElementSet elements)
    {
        ...
        // get the current shared params definition file
        DefinitionFile sharedParamsFile 
                    = SharedParameter.GetSharedParamsFile(app);
        if (null == sharedParamsFile)
        {
            TaskDialog.Show(
                                "Per document parameter", 
        ...
        // get or create the shared params group
        DefinitionGroup sharedParamsGroup 
            = SharedParameter.GetOrCreateSharedParamsGroup
                                        (
                                            sharedParamsFile, 
                                            kParamGroupName
                                        );
        ...
        // visible param
        Definition docParamDefVisible 
            = SharedParameter.GetOrCreateSharedParamsDefinition
                                    (
                                        sharedParamsGroup, 
        ...
        if (null == docParamDefVisible)
        {
            TaskDialog.Show (
                                "Per document parameter", 
                                "Error creating visible per-doc parameter."
                            );
            return Result.Failed;
        }
        // invisible param
        Definition docParamDefInvisible 
            = SharedParameter.GetOrCreateSharedParamsDefinition
                                    (
                                        sharedParamsGroup, 
                                        ParameterType.Integer, 
        ...
        if (null == docParamDefInvisible)
        {
            ...
            (
                "Per document parameter", 
                "Error creating invisible per-doc parameter."
            );
            return Result.Failed;
        }
        ...
        catch (Exception e)
        {
            TaskDialog.Show
                        (
                            "Per document parameter", 
                            "Error binding shared parameter: " + e.Message
                        );
            return Result.Failed;
        }
        ...
        if (null == projInfoElem)
        {
            TaskDialog.Show
                (
                    "Per document parameter", 
                    "No project info elem found. Aborting command..."
                );
            return Result.Failed;
        }
    ...
    /// using Revit 2009 filtering by searching for the 
    ...
    public static Element GetProjectInfoElem(Document doc)
    {
        ...
        Debug.Assert(elems.Count == 1, 
            "There should be exactly one of this object in the project"); 
