...
namespace Revit.SDK.Samples.DoorSwing.CS
{
   ...
   public class DoorFamily
   {
      ...
      public string BasalOpeningValue
      {
         get
         {
            if (string.IsNullOrEmpty(m_basalOpeningValue))
            {
               ...
               FamilySymbolSet fss = new FamilySymbolSet();
               foreach (ElementId elementId in m_family.GetFamilySymbolIds())
               {
                  fss.Insert((FamilySymbol)(m_app.ActiveUIDocument.Document.GetElement(elementId)));
               }
               FamilySymbolSetIterator doorSymbolIter = fss.ForwardIterator();
      ...
      public void UpdateOpeningFeature()
      {
         ...
         FamilySymbolSet ffs = new FamilySymbolSet();
         foreach (ElementId elementId in m_family.GetFamilySymbolIds())
         {
            ffs.Insert((FamilySymbol)(m_app.ActiveUIDocument.Document.GetElement(elementId)));
         }
         FamilySymbolSetIterator doorSymbolIter = ffs.ForwardIterator();
