  ...
  public class Command : IExternalCommand
  {
                                                ...
                                                ref string message, ElementSet elements )
    {
      try
      {
        ...
        MessageBox.Show( "Create view section successfully", "Revit" );
    ...
    Boolean GetSelectedElement()
    {
      ...
      {
        m_type = SelectType.BEAM;       // when the elemnt is a beam
        return true;
      }

      if( m_currentComponent is Floor )
      {
        m_type = SelectType.FLOOR;      // when the elemnt is a floor.
        return true;
      }
    ...
    Boolean GenerateBoundingBoxXYZ()
    {
      ...
      // It defines the orgin and the directions (including RightDirection, 
    ...
    Transform GenerateFloorTransform()
    {
      ...
      if( null == model )
      {
        m_errorInformation = "Please select a structual floor.";
        return transform;
      }
  ...
  public class CreateDraftingView : IExternalCommand
  {
      ...
      ElementSet elements )
    {
      try
      {
        ...
        if( null == drafting )
        {
          message = "Can't create the drafting view.";
          return IExternalCommand.Result.Failed;
        }
        MessageBox.Show( "Create drafting view succeeded.", "Revit" );
        return IExternalCommand.Result.Succeeded;
      }
      catch( Exception e )
      {
