...
namespace Revit.SDK.Samples.ShaftHolePuncher.CS
{
    ...
    public class ProfileNull : Profile
    {
        ...
        {
            Type type = typeof(Autodesk.Revit.Elements.Level);
            ElementFilterIterator iter = 
                m_commandData.Application.ActiveDocument.get_Elements(type);
            iter.Reset();
            while (iter.MoveNext())
            {
                Level level = iter.Current as Level;
                if ("Level 1" == level.Name)
                {
                    level1 = level;
                }
                if ("Level 2" == level.Name)
                {
                    level2 = level;
                }
            }
