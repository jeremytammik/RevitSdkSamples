...
namespace Revit.SDK.Samples.ShaftHolePuncher.CS
{
    ...
    public class ProfileBeam : Profile
    {
        ...
        public override List<XYZArray> GetNeedPoints(List<List<Edge>> faces)
        {
            ...
            for (int i = 0; i < faces.Count; i++)
            {
                foreach (Edge edge in faces[i])
                {
                    ...
                    {
                        XYZArray transformedPoints = new XYZArray();
                        for (int j = 0; j < edgexyzs.Size; j++)
                        {
                            XYZ xyz = edgexyzs.get_Item(j);
                            XYZ transformedXYZ = m_beamTransform.OfPoint(ref xyz);
                            transformedPoints.Append(ref transformedXYZ);
                        }
        ...
        public override Matrix4 GetTo2DMatrix()
        {
            ...
            Vector4 vOrigin =  new Vector4(m_points[0].get_Item(0));
        ...
        public override List<List<Edge>> GetFaces(Autodesk.Revit.Element elem)
        {
            ...
            foreach (GeometryObject geo in gObjects)
            {
                ...
                if (geo is Autodesk.Revit.Geometry.Instance)
                {
                    Autodesk.Revit.Geometry.Instance instance = geo as Autodesk.Revit.Geometry.Instance;
                    m_beamTransform = instance.Transform;
                    Autodesk.Revit.Geometry.Element elemGeo = instance.SymbolGeometry;
                ...
                else if (geo is Autodesk.Revit.Geometry.Solid)
                {
                    m_haveOpening = true;
        ...
        public override Opening CreateOpening(List<Vector4> points)
        {
            ...
            for (int i = 0; i < points.Count - 1; i++)
            {
                ...
                curve = m_appCreator.NewLine(ref p1, ref p2, true);
                curves.Append(curve);
            }
            ...
            if (false == m_isZaxis)
            {
                return m_docCreator.NewOpening(m_data, curves, Autodesk.Revit.Creation.Document.eRefFace.CenterY);
            }
            else
            {
                return m_docCreator.NewOpening(m_data, curves, Autodesk.Revit.Creation.Document.eRefFace.CenterZ);
            }
