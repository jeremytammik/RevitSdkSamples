...
namespace Revit.SDK.Samples.FoundationSlab.CS
{
    ...
    public class RegularSlab
    {
        ...
        Symbol m_type; // The type of the slab.
        ...
        public RegularSlab(Floor floor,CurveArray floorProfile, BoundingBoxXYZ bBox)
        {
            ...
            if (null != floor)
            {
                ...
                m_level = floor.Level;   // Get floor's level.
                m_type = floor.ObjectType;  // Get floor's type.
        ...
        private bool CreateOctagonProfile(XYZ min, XYZ max)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                Line line;
                if (7 == i)
                    line = SlabData.CreApp.NewLine(ref points[i], ref points[0],true);
                else
                    line = SlabData.CreApp.NewLine(ref points[i], ref points[i + 1],true);
                m_octagonalProfile.Append(line);
            }

            // Get the octagonal profile's bounding box.
            XYZ newMin = new XYZ(min.X - xOffset,min.Y - yOffset,z);
            XYZ newMax = new XYZ(max.X + xOffset,max.Y + yOffset,z);
