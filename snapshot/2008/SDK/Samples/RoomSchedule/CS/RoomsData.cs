...
namespace Revit.SDK.Samples.RoomSchedule
{
    ...
    public class RoomsData
    {
        ...
        // Store the reference of the Revit application 
        Autodesk.Revit.Application m_revit;
        ...
        public DataTable GenRoomsDataTable(Level level)
        {
            ...
            foreach (Room room in m_rooms)
            {
                ...
                if ((null == level) || (room.Level != null && room.Level.Id.Value == level.Id.Value))
                {
                    DataRow dataRow = newTable.NewRow();
                    for (int i = 0; i < m_parameters.Count; i++)
                    {
                        dataRow[i] = GetProperty(room, m_parameters[i], true);
                    }
        ...
        public String GetProperty(Room room, BuiltInParameter paraEnum, bool useValue)
        {
            ...
            switch (storageType)
            {
                ...
                case StorageType.ElementId:
                    ...
                    Element elem = m_revit.ActiveDocument.get_Element(ref elemId);
        ...
        public static bool ShareParameterExists(Room roomObj, String paramName, ref Parameter sharedParam)
        {
            ...
            // get the parameter
            foreach (Parameter param in roomObj.Parameters)
            {
                if(null == param) 
                {
                    continue;
                ...
                if (null != paramDef && String.Compare(paramDef.Name, paramName) == 0)
                {
                    sharedParam = param;
                    return true;
                }
        ...
        private void GetAllRooms()
        {
            ...
            ElementFilterIterator roomFileter = m_revit.ActiveDocument.get_Elements(typeof(Room));
            roomFileter.Reset();

            // try to find all rooms in the project and add to the list
            while (roomFileter.MoveNext())
            {
                // find the rooms and skip the rooms which are not "Rooms" category, 
                // such as "Areas" category, area is Room element too, but we should skip them.
                Room tmpRoom = roomFileter.Current as Room;
                if (null != tmpRoom && 0 == String.Compare(tmpRoom.Category.Name, "Rooms"))
                {
                    m_rooms.Add(tmpRoom);
                }
            }
        ...
        private void InitializeParameters()
        {
            ...
            // Room Perimeter
            m_parameters.Add(BuiltInParameter.ROOM_PERIMETER);
            m_columnNames.Add("Perimeter");
