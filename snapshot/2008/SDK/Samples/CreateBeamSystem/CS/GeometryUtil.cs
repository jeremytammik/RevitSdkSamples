...
namespace Revit.SDK.Samples.CreateBeamSystem.CS
{
	...
	public class GeometryUtil
	{
		...
		public static List<Line> SortLines(List<Line> originLines)
		{
			...
			result.Add(lines[0]);
			XYZ intersectPnt = lines[0].get_EndPoint(1);
			lines[0]         = null;
			...
			{	
				for (int j = 1; j < lines.Count; j++)
				{
					...
					{
						XYZ startPnt = lines[j].get_EndPoint(1);
						XYZ endPnt = lines[j].get_EndPoint(0);
						lines[j] = null;
						Line inversedLine = CreApp.NewLine(ref startPnt, ref endPnt, true);
		...
		public static bool InSameHorizontalPlane(List<Line> lines)
		{
			...
			XYZ firstPnt = lines[0].get_EndPoint(0);
			for (int i = 0; i < lines.Count; i++)
			{
