...
namespace Revit.SDK.Samples.Openings.CS
{
    ...
    {
        private Application m_revit; //Application of Revit
        ...
        public string NameAndId
        {
            get
            {
                return String.Concat(m_opening.Name, " (", m_opening.Id.Value.ToString(), ")");
            }
        }
        ...
        public OpeningInfo(Opening opening, Application app)
        {
            m_opening = opening;
            m_revit = app;
            ...
            BoundingBoxXYZ boxXYZ = m_opening.get_BoundingBox(m_revit.ActiveDocument.ActiveView);
        ...
        private void GetProfile()
        {
            ...
            if (null != curveArray)
            {
                ...
                {
                    //if curve is a circle, can not get the curve
                    if (!curve.IsBound)
                    {
                        m_sketch = null;
                        return;
            ...
            else if (m_opening.IsRectBoundary)
            {
                ...
                m_lines.Clear();
                XYZArray boundRect = m_opening.BoundaryRect;
        ...
        /// </summary>
        /// <param name="boundRect">an array contain two XYZ struct store the max and min 
        /// coordinate of rectangular</param>
        private XYZArray GetPoints(XYZArray boundRect)
        {
            XYZArray points = new XYZArray();
            XYZ p1 = boundRect.get_Item(0);
            points.Append(ref p1);

            XYZ p2 = new XYZ();
            p2.X = boundRect.get_Item(0).X;
            p2.Y = boundRect.get_Item(0).Y;
            p2.Z = boundRect.get_Item(1).Z;
            points.Append(ref p2);

            XYZ p3 = boundRect.get_Item(1);
            points.Append(ref p3);

            XYZ p4 = new XYZ();
            p4.X = boundRect.get_Item(1).X;
            p4.Y = boundRect.get_Item(1).Y;
            p4.Z = boundRect.get_Item(0).Z;
            points.Append(ref p4);

            //make rectangle close
            XYZ p5 = boundRect.get_Item(0);
            points.Append(ref p5);

            return points;
        }

        /// <summary>
        /// get line from XYZArray(points) and add line to m_lines list
        /// </summary>
        /// <param name="points">a XYZArray contain points of the Curve</param>
        private void AddLine(XYZArray points)
        {
            ...
            Autodesk.Revit.Geometry.XYZ previousPoint;
            previousPoint = points.get_Item(0);

            for (int i = 1; i < points.Size; i++)
            {
                Autodesk.Revit.Geometry.XYZ point;
