...
namespace Revit.SDK.Samples.CreateWallinBeamProfile.CS
{
    ...
    public class CreateWallinBeamProfile : IExternalCommand
    {
        ...
        /// <param name="project">A reference of current document</param>
        /// <returns>true if no error happens; otherwise, false.</returns>
        Boolean PrepareData(Autodesk.Revit.Document project)
        {
            ...
            AnalyticalModelFrame model;     // store the AnalyticalModel of the beam.
            foreach (Autodesk.Revit.Element e in selection)
            {
                ...
                if (null != m && StructuralType.Beam == m.StructuralType)
                {
                    ...
                    model = m.AnalyticalModel as AnalyticalModelFrame;
                    if (null == model)
                    {
                        m_errorInformation = "The beam should have analytical model frame.";
                        return false;
                    }
                    m_analyticalLineCollection.Add(model.Curve);
                }
            }
            ...
            ElementIterator iterator = project.Elements;
            iterator.Reset();
            while (iterator.MoveNext())
            {
                Level l = iterator.Current as Level;
                if (null != l)
                {
                    m_level = l;
                }
        ...
        Boolean EqualPoint(XYZ first, XYZ second)
        {
