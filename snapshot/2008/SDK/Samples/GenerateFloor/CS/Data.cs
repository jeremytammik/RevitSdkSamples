...
namespace Revit.SDK.Samples.GenerateFloor.CS
{
    ...
    public class Data
    {
        ...
        public void ObtainData(ExternalCommandData commandData)
        {
            ...
            m_creApp = commandData.Application.Create;
            Profile = m_creApp.NewCurveArray();


            ObtainFloorTypes(doc.Elements);
            ObtainProfile(walls);
            ObtainLevel(walls);
            Generate2D();
            Structural = true;
        ...
        private void ObtainFloorTypes(ElementIterator elements)
        {
            ...
            {
                Autodesk.Revit.Symbols.FloorType ft = elements.Current as Autodesk.Revit.Symbols.FloorType;
        ...
        {
            Autodesk.Revit.Elements.Level temp = null;
        ...
        private void Generate2D()
        {
            ...
            {
                XYZArray xyzArray = c.Tessellate();
                foreach (XYZ xyz in xyzArray)
                {
                    XYZ temp = new XYZ(xyz.X, -xyz.Y, xyz.Z);
        ...
        static private ElementSet WallFilter(ElementSet miscellanea)
        {
            ...
            foreach (Autodesk.Revit.Element e in miscellanea)
            {
                Wall w = e as Wall;
                if (null != w)
                {
        ...
        {
            XYZ temp = lines.get_Item(0).get_EndPoint(1);
        ...
        private Curve GetNext(CurveArray profile, XYZ connected, Curve line)
        {
            foreach (Curve c in profile)
            {
                ...
                else if (Math.Abs(c.get_EndPoint(1).X - connected.X) < PRECISION && Math.Abs(c.get_EndPoint(1).Y - connected.Y) < PRECISION && Math.Abs(c.get_EndPoint(1).Z - connected.Z) < PRECISION)
                {
                    ...
                    {
                        XYZ start = c.get_EndPoint(1);
                        XYZ end = c.get_EndPoint(0);
                        return m_creApp.NewLineBound(ref start, ref end);
                    }
                    else if (c.GetType().Name.Equals("Arc"))
                    {
                        int size = c.Tessellate().Size;
                        XYZ start = c.Tessellate().get_Item(0);
                        XYZ middle = c.Tessellate().get_Item(size / 2);
                        XYZ end = c.Tessellate().get_Item(size);

                        return m_creApp.NewArc(ref start, ref end, ref middle);
                    }
                }
            }
