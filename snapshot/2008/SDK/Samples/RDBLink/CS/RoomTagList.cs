...
namespace Revit.SDK.Samples.RDBLink.CS
{
    ...
    /// </summary>
    public class RoomTagList : NonCreatableElementList
    {
        #region Constructors
        /// <summary>
        ...
        {
            RoomTag rt = element as RoomTag;
            row["Id"] = GetIdDbValue(rt);
            row["RoomTagType"] = GetNameDbValue(rt.ObjectType);
            row["RoomId"] = GetIdDbValue(rt.Room);
            row["ViewId"] = GetIdDbValue(rt.View);
        } 
        #endregion
    }
}
