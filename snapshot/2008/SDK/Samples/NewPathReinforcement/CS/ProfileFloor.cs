...
namespace Revit.SDK.Samples.NewPathReinforcement.CS
{
    ...
    public class ProfileFloor : Profile
    {
        ...
        public override Matrix4 GetTo2DMatrix()
        {
            ...
            ElementFilterIterator iter = m_commandData.Application.ActiveDocument.get_Elements(type);
            iter.Reset();
            while (iter.MoveNext())
            {
            ...
            //Because Y axis in windows UI is downward, so we should Multipy(-1) here
        ...
        public override PathReinforcement CreatePathReinforcement(List<Vector4> points, bool flip)
        {
            ...
            return m_docCreator.NewPathReinforcement(pathReinforcementType, m_data, curves, flip);
        }
    }
}
