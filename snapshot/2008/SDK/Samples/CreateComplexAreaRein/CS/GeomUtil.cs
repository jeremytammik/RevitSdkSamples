...
namespace Revit.SDK.Samples.CreateComplexAreaRein.CS
{
	...
	class GeomUtil
	{
		...
		public static FaceArray GetFaces(Element elem)
		{
			...
			Autodesk.Revit.Geometry.Options geoOptions = Command.CommandData.Application.Create.NewGeometryOptions();
		...
		{
            XYZ sub = SubXYZ(line.get_EndPoint(0), line.get_EndPoint(1));
		...
		{
            XYZ direct = SubXYZ(inLine.get_EndPoint(1), inLine.get_EndPoint(0));
			XYZ dPerp = new XYZ();
			dPerp.X = -direct.Y;
			dPerp.Y = direct.X;
			double length = Math.Sqrt(dPerp.X * dPerp.X + dPerp.Y * dPerp.Y);
			double temp = distance / length;
			dPerp.X = dPerp.X * temp;
			dPerp.Y = dPerp.Y * temp;
			dPerp.Z = 0.0;

            XYZ startPoint = AddXYZ(inLine.get_EndPoint(0), dPerp);
            XYZ endPoint = AddXYZ(inLine.get_EndPoint(1), dPerp);
            Line outLine = Line.get_Bound(ref startPoint, ref endPoint);
		...
		{
            XYZ startPoint = inLine.get_EndPoint(0);
            XYZ endPoint = inLine.get_EndPoint(1);

			XYZ temp1 = SubXYZ(endPoint, startPoint);
			XYZ temp2 = MultiXYZ(temp1, (scale - 1) / 2);
			XYZ startPoint2 = SubXYZ(startPoint, temp2);
			XYZ endPoint2 = AddXYZ(endPoint, temp2);

            Line outLine = Line.get_Bound(ref startPoint2, ref endPoint2);
		...
		/// <summary>
		/// judge whether the subtraction of two doubles is less than the internal dicided precision
		/// </summary>
		/// <param name="d1"></param>
		/// <param name="d2"></param>
		/// <returns></returns>
