    ...
    public class Command : IExternalCommand
    {
            ...
            ElementSet elements)
        {
            try
            {
                ...
                {
                  System.Windows.Forms.DialogResult result = displayForm.ShowDialog();
      ...
      try
      {
        ...
        foreach( Element elem in doc.Selection.Elements )
        {
          if( elem is Rebar )
          {
            ...
            IndependentTag tag = doc.Create.NewTag( view, rebar, true,
                TagMode.TM_ADDBY_CATEGORY, TagOrientation.TAG_HORIZONTAL, 
                curve.get_EndPoint( 0 ) );
            return IExternalCommand.Result.Succeeded;
          }
        }
      ...
      try
      {
        ...
        foreach( Element elem in doc.Selection.Elements )
        {
          if( elem is Rebar )
          {
            ...
            // create the text
            TextNote text = doc.Create.NewTextNote( view, origin, baseVec, upVec, textSize, lineWidth,
                TextAlignFlags.TEF_ALIGN_CENTER | TextAlignFlags.TEF_ALIGN_MIDDLE, strText );
            text.Width = curve.Length * 10; // set the text width
            return IExternalCommand.Result.Succeeded;
          }
        }
        message = "No rebar selected!";
