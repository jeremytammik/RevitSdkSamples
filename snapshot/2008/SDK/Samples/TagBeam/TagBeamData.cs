...
namespace Revit.SDK.Samples.TagBeam.CS
{
    ...
    public class TagBeamData
    {
        ...
        private Autodesk.Revit.Elements.View m_view; // current view
        ...
        public TagBeamData(ExternalCommandData commandData)
        {
            ...
            m_docCreator = revitDoc.Create;
            m_view = revitDoc.ActiveView;

            Autodesk.Revit.SelElementSet elementSet = revitDoc.Selection.Elements;
            ElementSetIterator itor = elementSet.ForwardIterator();
            while (itor.MoveNext())
            {
                FamilyInstance familyInstance = itor.Current as FamilyInstance;
                if ((familyInstance != null) && (familyInstance.StructuralType == Autodesk.Revit.Structural.Enums.StructuralType.Beam))
                {
            ...
            ElementIterator eitor = revitDoc.get_Elements(typeof(Family));
            eitor.Reset();
            while(eitor.MoveNext())
            {
                Family family = eitor.Current as Family;
            ...
            TagOrientation tagOrientation)
        {
            foreach(FamilyInstance beam in m_beamList)
            {
                ...
                Autodesk.Revit.LocationCurve location = beam.Location as Autodesk.Revit.LocationCurve;
                Autodesk.Revit.Geometry.Curve curve = location.Curve;
                ...
                tag1.ObjectType = tag2.ObjectType = tagSymbol.FamilySymbol;
            }
