...
namespace RevitLookup.Utils {
    ...
    public class Elements {
        ...
        {
            BeamSystem beamSystemClone = app.ActiveUIDocument.Document.Create.NewBeamSystem(beamSystem.Profile, beamSystem.Level);
        ...
        private static Revit.Element CloneElement(Autodesk.Revit.UI.UIApplication app, Wall wall)
        {
            ...
            Wall wallClone = app.ActiveUIDocument.Document.Create.NewWall(locCurve.Curve, wall.Level, isStructural);
        ...
        private static Revit.Element CloneElement(Autodesk.Revit.UI.UIApplication app, FamilyInstance familyInstance)
        {
            ...
            if (locationCurve != null) {
                location = locationCurve.Curve.get_EndPoint(0);
            }
        ...
        private static Revit.Element CloneElement(Autodesk.Revit.UI.UIApplication app, Floor floor)
        {
            ...
            Floor floorClone = app.ActiveUIDocument.Document.Create.NewFloor(curveArray, floor.FloorType, floor.Level, false);
        ...
        {
            XYZ startPt = dimension.Curve.get_EndPoint(0);
            XYZ endPt = dimension.Curve.get_EndPoint(1);
            Line line = app.Application.Create.NewLine(startPt, endPt, true);
        ...
        {
            Revit.ElementSet elemSet = app.Application.Create.NewElementSet();
            Revit.ElementArray elemArray = group.Members;
            Revit.ElementArrayIterator elemArrayIter = elemArray.ForwardIterator();
            while (elemArrayIter.MoveNext()) {
                Revit.Element elem = elemArrayIter.Current as Revit.Element;
                elemSet.Insert(elem);
            }
            Group groupClone = app.ActiveUIDocument.Document.Create.NewGroup(elemSet);
        ...
        private static Revit.Element CloneElement(Autodesk.Revit.UI.UIApplication app, Autodesk.Revit.DB.Architecture.Room room)
        {
            ...
            Room roomClone = app.ActiveUIDocument.Document.Create.NewRoom(room.Level, point);
        ...
        {
            SketchPlane sketchPlaneClone = app.ActiveUIDocument.Document.Create.NewSketchPlane(sketchPlane.Plane);
        ...
        private static Revit.Element CloneElement(Autodesk.Revit.UI.UIApplication app, View3D view3D)
        {
            ...
            View3D view3DClone = app.ActiveUIDocument.Document.Create.NewView3D(viewDir);
