...
namespace FamilyCs
{
  ...
  class RvtCmd_FamilyCreateColumnVisibility : IExternalCommand
  {
    ...
    Extrusion createSolid()
    {
      ...
      SketchPlane pSketchPlane = _doc.FamilyCreate.NewSketchPlane(pRefPlane.Plane);
    ...
    CurveArrArray createProfileLShape()
    {
      ...
      {
        Line line = _app.Create.NewLineBound(pts[i], pts[i + 1]);
        pLoop.Append(line);
      }
    ...
    void addDimensions()
    {
      ...
      Line pLine = _app.Create.NewLineBound(p0, p1);
    ...
    public void addLineObjects()
    {
      ...
      SketchPlane pSketchPlaneH = _doc.FamilyCreate.NewSketchPlane(pGeomPlaneH);
        ...
        if (geomObj is Solid) // solid is what we are interested in.
        {
          ...
          foreach (Face pFace in faces)
          {
            ...
            if ((pPlanarFace != null) && pPlanarFace.Normal.IsAlmostEqualTo(normal))
            {
              ...
              Line pCurve = _app.Create.NewLineBound(p0, p1);
