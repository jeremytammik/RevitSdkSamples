...
namespace Revit.SDK.Samples.CreateViewSection.CS
{
    ...
    public class Command : IExternalCommand
    {
        ...
        Boolean GetSelectedElement()
        {
            ...
            if( null != beam && StructuralType.Column == beam.StructuralType ) // added by jeremy
            {
              m_type = SelectType.COLUMN;       // when the element is a column
              return true;
            }
        ...
        Boolean GenerateBoundingBoxXYZ()
        {
          ...
          m_box.Min = minPoint;
            ...
            // It define the Origin and the directions (include RightDirection, 
        ...
        Transform GenerateTransform()
        {
            ...
            else if( SelectType.COLUMN == m_type )
            {
              return GenerateColumnTransform();
            }
        ...
        Transform GenerateWallTransform()
        {
            ...
            XYZ mPoint = XYZMath.FindMidPoint(locationLine.get_EndPoint(0), locationLine.get_EndPoint(1));
        ...
        /// </summary>
        /// <returns>the reference of Transform, return null if it can't be generated</returns>
        Transform GenerateColumnTransform() // added by jeremy
        {
          Transform transform = null;
          FamilyInstance instance = m_currentComponent as FamilyInstance;

          // get the Analytical Model line.
          AnalyticalModel model = instance.GetAnalyticalModel();
          if( null == model )
          {
            m_errorInformation = "The selected column doesn't have Analytical Model line.";
            return transform;
          }
          Curve curve = model.GetCurve();
          if( null == curve )
          ...
          // At last find out the directions of the created view, and set it as Basis property.   
          XYZ basisZ = XYZMath.FindDirection( startPoint, endPoint );
          XYZ basisX = XYZ.BasisX; // added by jeremy
          XYZ basisY = XYZ.BasisY; // added by jeremy
