        ...
        private DataTable StoreInformationInDataTable(ElementSet selectedElements)
        {
            ...
            foreach (Element element in selectedElements)
            {
                ...
                case "ContFooting":
                    ...
                    ElementType wallFootSymbol =m_revit.Application.ActiveUIDocument.Document.get_Element(wallFoot.GetTypeId()) as ElementType;// get element Type
                    typeName              = wallFootSymbol.Category.Name + ": " + wallFootSymbol.Name;
                    break;

                case "FamilyInstance":
                    FamilyInstance familyInstance = element as FamilyInstance;
                    FamilySymbol symbol = m_revit.Application.ActiveUIDocument.Document.get_Element(familyInstance.GetTypeId()) as FamilySymbol;
                    typeName                      = symbol.Family.Name + ": " + symbol.Name;
                    break;

                case "Floor":
                    Floor slab         = element as Floor;                    
                    FloorType slabType = m_revit.Application.ActiveUIDocument.Document.get_Element(slab.GetTypeId()) as FloorType; // get element type
                    typeName           = slabType.Category.Name + ": " + slabType.Name;
                    break;

                case "Wall":
                    Wall wall         = element as Wall;  
                    WallType wallType = m_revit.Application.ActiveUIDocument.Document.get_Element(wall.GetTypeId()) as WallType; // get element type
