...
namespace Revit.SDK.Samples.ImportExport.CS
{
    ...
    public class ExportDWGData : ExportDataWithViews
    {
        ...
        void rename()
        {
            try
            {
                string[] Files = Directory.GetFiles(m_exportFolder);
                foreach (string File in Files)
                {
                    foreach (View view in m_selectViewsData.SelectedViews)
                    {
                        //System.Windows.Forms.MessageBox.Show(File + "\nChecking for: " + view.ViewName.Replace(" ", ""));
                        if (File.Contains(view.ViewName.Replace(" ", "")))
                        {
                            System.IO.File.Move(File, m_exportFolder + "\\" + m_exportFileName.Replace("REPLACE", view.ViewName.Replace(" ", "")).ToString() + File.Substring(File.Length - 4, 4));
                            continue;
                            //System.Windows.Forms.MessageBox.Show("Replaced:\n" + File + " with: " + m_exportFolder + "\\" + m_exportFileName.Replace("REPLACE", view.ViewName).ToString() + File.Substring(File.Length - 4, 4));
                        }
                    }
                }
            }
