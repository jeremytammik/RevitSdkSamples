  ...
  public class GeometrySupport
  {
    ...
    public GeometrySupport( FamilyInstance element )
    {
      ...
      if (null != line)
      {
          ...
          m_drivingVector = line.GetEndPoint(1) - line.GetEndPoint(0);
    ...
    protected List<XYZ> GetRelatedVectors( XYZ point )
    {
      ...
      {
        if( IsEqual( point, line.GetEndPoint( 0 ) ) )
        {
          XYZ vector = SubXYZ( line.GetEndPoint( 1 ), line.GetEndPoint( 0 ) );
          vectors.Add( vector );
        }
        if( IsEqual( point, line.GetEndPoint( 1 ) ) )
        {
          XYZ vector = SubXYZ( line.GetEndPoint( 0 ), line.GetEndPoint( 1 ) );
          vectors.Add( vector );
        }
      }
    ...
    private List<Line> ChangeEdgeToLine( EdgeArray edges )
    {
      ...
      foreach( Edge edge in edges )
      {
        ...
        //edgeLines.Add( Line.get_Bound( first, second ) ); // 2013
        edgeLines.Add( Line.CreateBound( first, second ) ); // 2014
      }
