  ...
  public class Lab3_ListSelectedAnalyticalModels : IExternalCommand
  {
    ...
    public void ListCurve( ref string s, Curve crv )
    {
      ...
      if( crv is Line )
      {
        ...
        s += "  LINE: " + RacUtils.PointString( line.GetEndPoint( 0 ) )
          + " ; " + RacUtils.PointString( line.GetEndPoint( 1 ) ) + "\r\n";
      }
      else if( crv is Arc )
      {
        Arc arc = crv as Arc;
        s += "  ARC: " + RacUtils.PointString( arc.GetEndPoint( 0 ) )
          + " ; " + RacUtils.PointString( arc.GetEndPoint( 1 ) )
      ...
      else
      {
        ...
        if( listSupports.Count > 0 )
        {
          ...
          foreach( AnalyticalModelSupport supInfo in listSupports )
          {
            ...
            Element elemSupport = m_doc.GetElement( idSupport );
